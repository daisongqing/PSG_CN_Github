...
// HISTORY 22-12-2009 Z-1649 Added AES support 22-02-2010 Z-1648 Zero byte entries would create
// invalid zip files
...
{
    /// <summary>
    /// This is a DeflaterOutputStream that writes the files into a zip archive one after another. It
    /// has a special method to start a new zip entry. The zip entries contains information about the
    ...
    public class ZipOutputStream : DeflaterOutputStream
    {
        ...
        /// <summary>
        /// Gets a flag value of true if the central header has been added for this archive; false if
        ...
        /// </summary>
        /// <remarks>
        /// Older archivers may not understand Zip64 extensions. If backwards compatability is an
        /// issue be careful when adding <see cref="ZipEntry.Size"> entries </see> to an archive.
        /// Setting this property to off is workable but less desirable as in those circumstances
        /// adding a file larger then 4GB will fail.
        /// </remarks>
        ...
        /// <summary>
        /// Starts a new Zip entry. It automatically closes the previous entry if present. All entry
        /// elements bar name are optional, but must be correct if present. If the compression method
        /// is stored and the output is not patchable the compression for that entry is automatically
        /// changed to deflate level 0
        /// </summary>
        /// <param name="entry"> the entry. </param>
        /// <exception cref="System.ArgumentNullException"> if entry passed is null. </exception>
        ...
        /// Too many entries in the Zip file <br/> Entry name is too long <br/> Finish has already
        /// been called <br/>
        /// </exception>
        public void PutNextEntry(ZipEntry entry)
        {
            ...
            if (Password != null)
            {
                ...
                if (entry.Crc < 0)
                {
                    // Need to append a data descriptor as the crc isnt available for use with
                    // encryption, the date is used instead. Setting the flag indicates this to the decompressor.
                    entry.Flags |= 8;
                }
            }
        ...
        private void WriteAESHeader(ZipEntry entry)
        {
            ...
            // ------------   ------- Variable Salt value 2 Password verification value Variable
            // Encrypted file data 10 Authentication code
            //
            // Value in the "compressed size" fields of the local file header and the central
            // directory entry is the total size of all the items listed above. In other words, it is
            // the total size of the salt value, password verification value, encrypted data, and
            // authentication code.
        ...
        /// <summary>
        /// Finishes the stream. This will write the central directory at the end of the zip file and
        /// flush the stream.
        /// </summary>
        /// <remarks> This is automatically called when the stream is closed. </remarks>
        /// <exception cref="System.IO.IOException"> An I/O error occurs. </exception>
        /// <exception cref="ZipException">
        /// Comment exceeds the maximum length <br/> Entry name exceeds the maximum length
        /// </exception>
