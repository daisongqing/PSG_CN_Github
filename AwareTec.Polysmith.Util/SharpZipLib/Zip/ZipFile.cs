// ZipFile.cs
//
// Copyright (C) 2001 Mike Krueger Copyright (C) 2004 John Reilly
//
// This file was translated from java, it was part of the GNU Classpath Copyright (C) 2001 Free
...
// on this library. Thus, the terms and conditions of the GNU General Public License cover the whole combination.
//
// As a special exception, the copyright holders of this library give you permission to link this
// library with independent modules to produce an executable, regardless of the license terms of
// these independent modules, and to copy and distribute the resulting executable under terms of your
...
// based on this library. If you modify this library, you may extend this exception to your version
// of the library, but you are not obligated to do so. If you do not wish to do so, delete this
// exception statement from your version.

// HISTORY 2009-12-22 Z-1649 Added AES support 2010-03-02 Z-1650 Fixed updating ODT archives in
// memory. Exposed exceptions in updating. 2010-05-25 Z-1663 Fixed exception when testing local
// header compressed size of -1
...
namespace ICSharpCode.SharpZipLib.Zip
{
    ...
    /// <summary>
    /// Delegate invoked during <see cref="ZipFile.TestArchive(bool, TestStrategy,
    /// ZipTestResultHandler)"> testing </see> if supplied indicating current progress and status.
    /// </summary>
    /// <remarks>
    /// If the message is non-null an error has occured. If the message is null the operation as
    /// found in <see cref="TestStatus"> status </see> has started.
    /// </remarks>
    ...
    /// <summary>
    /// This class represents a Zip archive. You can ask for the contained entries, or get an input
    /// stream for a file entry. The entry is automatically decompressed.
    ///
    /// You can also update the archive adding or deleting entries.
    ///
    /// This class is thread safe for input: You can open input streams for arbitrary entries in
    /// different threads. <br/><br/> Author of the original java version : Jochen Hoenicke
    /// </summary>
    /// <example>
    /// <code>
    /// using System;
    ...
    public class ZipFile : IEnumerable, IDisposable
    {
        ...
        /// <summary>
        /// Closes the ZipFile. If the stream is <see cref="IsStreamOwner"> owned </see> then this
        /// also closes the underlying input stream. Once closed, no further instance methods should
        /// be called.
        /// </summary>
        ...
        /// <exception cref="ArgumentNullException">
        /// <paramref name="fileName"> </paramref> is null
        /// </exception>
        ...
        /// <summary>
        /// Get/set a flag indicating if the underlying stream is owned by the ZipFile instance. If
        ...
        /// <exception cref="ObjectDisposedException"> The Zip file has been closed. </exception>
        ...
        /// </param>
        /// <returns> A clone of the zip entry, or null if no entry with that name exists. </returns>
        /// <exception cref="ObjectDisposedException"> The Zip file has been closed. </exception>
        ...
        /// </summary>
        /// <param name="entry">
        /// The <see cref="ZipEntry"/> to obtain a data <see cref="Stream"/> for
        /// </param>
        /// <returns> An input <see cref="Stream"/> containing data for this <see cref="ZipEntry"/> </returns>
        /// <exception cref="ObjectDisposedException"> The ZipFile has already been closed </exception>
        ...
        /// <returns> An input <see cref="Stream"/> containing data for this <paramref name="entryIndex"/> </returns>
        /// <exception cref="ObjectDisposedException"> The ZipFile has already been closed </exception>
        /// <exception cref="ICSharpCode.SharpZipLib.Zip.ZipException">
        /// The compression method for the entry is unknown
        /// </exception>
        ...
        /// The <see cref="ZipTestResultHandler"> </see> handler to call during testing.
        /// </param>
        /// <returns> true if all tests pass, false otherwise </returns>
        ...
        public bool TestArchive(bool testData, TestStrategy strategy, ZipTestResultHandler resultHandler)
        {
            ...
            try
            {
                ...
                // TODO: the 'Corrina Johns' test where local headers are missing from the central
                // directory. They are therefore invisible to many archivers.
            }
            catch (Exception ex)
            {
                status.AddError();
        ...
        private long TestLocalHeader(ZipEntry entry, HeaderTest tests)
        {
            lock (baseStream_)
            {
                ...
                {
                    // 2010-03-04 Forum 10512: removed checks for version >=
                    // ZipConstants.VersionZip64 and size or compressedSize = MaxValue, due to rogue creators.
                ...
                if (testHeader)
                {
                    ...
                    // Crc valid for empty entry. This will also apply to streamed entries where size
                    // isnt known and the header cant be patched
                    ...
                    // Assuming a code page at this point is not valid? Best is to store the name
                    // length in the ZipEntry probably
                    ...
                    if (entry.IsDirectory)
                    {
                        ...
                        // There may be other cases where the compressed size can be greater than
                        // this? If so until details are known we will be strict.
                        if (entry.IsCrypted)
                        {
                        ...
                        {
                            // When not compressed the directory size can validly be 2 bytes if the
                            // true size wasnt known when data was originally being written.
                ...
                // Size can be verified only if it is known in the local header. it will always be
                // known in the central header.
                if (((localFlags & (int)GeneralBitFlags.Descriptor) == 0) ||
        ...
        /// The <see cref="IArchiveStorage"> archive storage </see> for use during the update.
        /// </param>
        /// <param name="dataSource">
        /// The <see cref="IDynamicDataSource"> data source </see> to utilise during updating.
        /// </param>
        ...
        /// <summary>
        /// Set the file comment to be recorded when the current update is <see cref="CommitUpdate">
        /// commited </see>.
        /// </summary>
        ...
        public void SetComment(string comment)
        {
            ...
            // We dont take account of the original and current comment appearing to be the same as
        ...
        /// The name to use for the <see cref="ZipEntry"/> on the Zip file created.
        /// </param>
        /// <exception cref="ArgumentNullException"> Argument supplied is null. </exception>
        ...
        */
        ...
        private void WriteLocalEntryHeader(ZipUpdate update)
        {
            ...
            if (entry.LocalHeaderRequiresZip64)
            {
                ...
                // Local entry header always includes size and compressed size. NOTE the order of
                // these fields is reversed when compared to the normal headers!
        ...
        private int FindExistingUpdate(ZipEntry entry)
        {
            ...
            /*
                        // This is slow like the coming of the next ice age but takes less storage
                        // and may be useful for CF?
        ...
        private void CopyEntryDirect(ZipFile workFile, ZipUpdate update, ref long destinationPosition)
        {
            ...
            if (skipOver)
            {
                if (update.OffsetBasedSize != -1)
                    ...
                    // WinZip produces a warning on these entries: "caution: value of lrec.csize
                    // (compressed size) changed from ..."
                    destinationPosition +=
        ...
        private void UpdateCommentOnly()
        {
            ...
            else
            {
                ...
                {
                    // TODO: archiveStorage wasnt originally intended for this use. Need to revisit
                    // this to tidy up handling as archive storage currently doesnt handle the
                    // original stream well. The problem is when using an existing zip archive with
                    // an in memory archive storage. The open stream wont support writing but the
                    // memory storage should open the same file not an in memory one.
        ...
        private void RunUpdates()
        {
            ...
            else if (archiveStorage_.UpdateMode == FileUpdateMode.Direct)
            {
                ...
                // Sort the updates by offset within copies/modifies, then adds. This ensures that
                // data required by copies will not be overwritten.
                updates_.Sort(new UpdateComparer());
            }
            else
            {
            ...
            try
            {
                ...
                foreach (ZipUpdate update in updates_)
                {
                    if (update != null)
                    {
                        // If the size of the entry is zero leave the crc as 0 as well. The
                        // calculated crc will be all bits on...
        ...
        private class ZipUpdate
        {
            ...
            /// <summary>
            /// Get/set the size calculated by offset. Specifically, the difference between this and
            /// next entry's starting offset.
            /// </summary>
        ...
        /// <summary>
        /// Releases the unmanaged resources used by the this instance and optionally releases the
        /// managed resources.
        /// </summary>
        ...
        /// <exception cref="EndOfStreamException"> The stream ends prematurely </exception>
        ...
        {
            // Search for the End Of Central Directory. When a zip comment is present the directory
            // will start earlier
            //
            // The search is limited to 64K which is the maximum size of a trailing comment field to
            // aid speed. This should be compatible with both SFX and ZIP files but has only been
            // tested for Zip files If a SFX file has the Zip data attached as a resource and there
            // are other resources occuring later then this could be invalid. Could also speed this
            // up by reading memory in larger blocks.
                ...
                (offsetOfCentralDir == 0xffffffff))
            {
                ...
                // relative offset of the zip64 end of central directory record 8 bytes total number
                // of disks 4 bytes
            ...
            // This applies to Zip files that are appended to the end of an SFX stub. Or are appended
            // as a resource to an executable. Zip files created by some archivers have the offsets
            // altered to reflect the true offsets and so dont require any adjustment here...
        ...
        /// <exception cref="ICSharpCode.SharpZipLib.Zip.ZipException">
        /// The local header signature is invalid, the entry and central header file name lengths are
        /// different or the local and entry compression methods dont match
        /// </exception>
        ...
        private Stream CreateAndInitEncryptionStream(Stream baseStream, ZipEntry entry)
        {
                ...
                || (entry.Flags & (int)GeneralBitFlags.StrongEncryption) == 0)
            {
                ...
                // Closing a CryptoStream will close the base stream as well so wrap it in an
                // UncompressedStream which doesnt do this.
        ...
        // Default is dynamic which is not backwards compatible and can cause problems with XP's
        // built in compression which cant read Zip64 archives. However it does avoid the situation
        // were a large file is added and cannot be completed correctly.
        ...
        private class ZipString
        {
            ...
            /// <summary>
            /// Get a value indicating the original source of data for this instance. True if the
            /// source was a string; false if the source was binary data.
            /// </summary>
        ...
        /// <summary>
        /// An <see cref="UncompressedStream"/> is a stream that you can write uncompressed data to
        /// and flush, but cannot read, seek or do anything else to.
        /// </summary>
        private class UncompressedStream : Stream
        {
            ...
            /// <param name="origin">
            /// A value of type <see cref="T:System.IO.SeekOrigin"> </see> indicating the reference
        ...
        /// <summary>
        /// A <see cref="PartialInputStream"/> is an <see cref="InflaterInputStream"/> whose data is
        /// only a part or subsection of a file.
        /// </summary>
        private class PartialInputStream : Stream
        {
            ...
            /// </summary>
            /// <param name="zipFile">
            /// The <see cref="ZipFile"/> containing the underlying stream to use for IO.
            /// </param>
            ...
            public PartialInputStream(ZipFile zipFile, long start, long length)
            {
                ...
                // Although this is the only time the zipfile is used keeping a reference here
                // prevents premature closure of this zip file and thus the baseStream_.

                // Code like this will cause apparently random failures depending on the size of the
                // files and when garbage is collected.
                //
                // ZipFile z = new ZipFile (stream); Stream reader = z.GetInputStream(0); uses reader here....
                zipFile_ = zipFile;
    ...
    /// <summary>
    /// Represents a source of data that can dynamically provide multiple <see cref="Stream"> data
    /// sources </see> based on the parameters passed.
    /// </summary>
    ...
    public interface IArchiveStorage
    {
        ...
        /// <returns>
        /// Returns a temporary output <see cref="Stream"/> that is a copy of the input.
        /// </returns>
    ...
    abstract public class BaseArchiveStorage : IArchiveStorage
    {
        ...
        /// </summary>
        /// <returns>
        /// Returns a <see cref="Stream"/> that can be used to read the final storage for the archive.
        /// </returns>
        /// <seealso cref="GetTemporaryOutput"/>
    ...
    public class DiskArchiveStorage : BaseArchiveStorage
    {
        ...
        public override Stream GetTemporaryOutput()
        {
            ...
            {
                // Determine where to place files based on internal strategy. Currently this is
                // always done in system temp directory.
    ...
    public class MemoryArchiveStorage : BaseArchiveStorage
    {
        ...
        /// <returns> Returns a stream suitable for direct updating. </returns>
        /// <remarks>
        /// If the <paramref name="stream"/> passed is not null this is used; otherwise a new <see
        /// cref="MemoryStream"/> is returned.
        /// </remarks>
        public override Stream OpenForDirectUpdate(Stream stream)
        {
            Stream result;
