...
namespace ICSharpCode.SharpZipLib.Zip.Compression
{
    ...
    public enum DeflateStrategy
    {
        ...
        /// <summary>
        /// This strategy will only allow longer string repetitions. It is useful for random data
        /// with a small character set.
        /// </summary>
        Filtered = 1,

        /// <summary>
        /// This strategy will not look for string repetitions at all. It only encodes with Huffman
        /// trees (which means, that more common characters get a smaller encoding.
        /// </summary>
        HuffmanOnly = 2
    }
    ...
    // DEFLATE ALGORITHM:
    //
    // The uncompressed stream is inserted into the window array. When the window array is full the
    // first half is thrown away and the second half is copied to the beginning.
    //
    // The head array is a hash table. Three characters build a hash value and they the value points
    // to the corresponding index in window of the last string with this hash. The prev array
    // implements a linked list of matches with the same hash: prev[index & WMASK] points to the
    // previous index with the same hash.

    /// <summary>
    /// Low level compression engine for deflate algorithm which uses a 32K sliding window with
    /// secondary compression from Huffman/Shannon-Fano codes.
    /// </summary>
    public class DeflaterEngine : DeflaterConstants
    {
        ...
        public DeflaterEngine(DeflaterPending pending)
        {
            ...
            // We start at index 1, to avoid an implementation deficiency, that we cannot build a
            // repeat pattern at index 0.
        ...
        /// window[index]..window[index+2] have this hash code. Note that the array should really be
        /// unsigned short, so you need to and the values with 0xffff.
        /// </summary>
        private short[] head;
        ...
        /// prev[index &amp; WMASK]
        /// </code>
        /// points to the previous index that has the same hash code as the string starting at index.
        /// This way entries with the same hash code are in a linked list. Note that the array should
        /// really be unsigned short, so you need to and the values with 0xffff.
        /// </summary>
        private short[] prev;
        ...
        /// <summary>
        /// lookahead is the number of characters starting at strstart in window that are valid. So
        /// window[strstart] until window[strstart+lookahead-1] are valid characters.
        /// </summary>
        private int lookahead;

        /// <summary>
        /// This array contains the part of the uncompressed stream that is of relevance. The current
        /// character is indexed by strstart.
        /// </summary>
        private byte[] window;
