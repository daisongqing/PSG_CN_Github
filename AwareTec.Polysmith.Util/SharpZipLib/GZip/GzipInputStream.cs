...
{
    /// <summary>
    /// This filter stream is used to decompress a "GZIP" format stream. The "GZIP" format is
    /// described baseInputStream RFC 1952.
    ///
    ...
    public class GZipInputStream : InflaterInputStream
    {
        ...
        /// <summary>
        /// Flag to indicate if we've read the GZIP header yet for the current member (block of
        /// compressed data). This is tracked per-block as the file is parsed.
        /// </summary>
        ...
        {
            // A GZIP file can contain multiple blocks of compressed data, although this is quite
            // rare. A compressed block could potentially be empty, so we need to loop until we reach
            // EOF or we find data.
            while (true)
            {
                ...
                if (!readGZIPHeader)
                {
                    // Try to read header. If there is no header (0 bytes available), this is EOF. If
                    // there is an incomplete header, this will throw an exception.
                    if (!ReadHeader())
                    {
                        return 0;
                    }
        ...
        private void ReadFooter()
        {
            ...
            // Mark header read as false so if another header exists, we'll continue reading through
            // the file
        ...
        private bool ReadHeader()
        {
            ...
            // Make sure there is data in file. We can't rely on ReadLeByte() to fill the buffer, as
            // this could be EOF, which is fine, but ReadLeByte() throws an exception if it doesn't
            // find data, so we do this part ourselves.
