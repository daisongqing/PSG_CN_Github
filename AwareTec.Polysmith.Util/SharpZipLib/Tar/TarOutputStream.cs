// TarOutputStream.cs
//
// Copyright (C) 2001 Mike Krueger Copyright 2005 John Reilly
//
...
namespace ICSharpCode.SharpZipLib.Tar
{
    ...
    public class TarOutputStream : Stream
    {
        ...
        /// <summary>
        /// Get/set flag indicating ownership of the underlying stream. When the flag is true <see
        /// cref="Close"> </see> will close the underlying stream also.
        /// </summary>
        ...
        /// </summary>
        /// <param name="offset">
        /// The offset relative to the <paramref name="origin"/> to seek to
        /// </param>
        ...
        /// <summary>
        /// Read a byte from the stream and advance the position within the stream by one byte or
        ...
        /// <summary>
        /// read bytes from the current stream and advance the position within the stream by the
        ...
        /// <returns>
        /// The total number of bytes read, or zero if at the end of the stream. The number of bytes
        /// may be less than the <paramref name="count"> count </paramref> requested if data is not avialable.
        /// </returns>
        ...
        /// <summary>
        /// Ends the TAR archive without closing the underlying OutputStream. The result is that the
        /// EOF block of nulls is written.
        /// </summary>
        public void Finish()
        {
            if (IsEntryOpen)
        ...
        /// <summary>
        /// Put an entry on the output stream. This writes the entry's header and positions the
        /// output stream for writing the contents of the entry. Once this method is called, the
        /// stream is ready for calls to write() to write the entry's contents. Once the contents are
        /// written, closeEntry() <B> MUST </B> be called to ensure that all buffered data is
        ...
        /// <summary>
        /// Close an entry. This method MUST be called for all file entries that contain data. The
        /// reason is that we must buffer data written to the stream in order to satisfy the buffer's
        /// block based writes. Thus, there may be data fragments still being assembled that must be
        /// written to the output stream before this entry is closed and the next entry written.
        /// </summary>
        ...
        /// <summary>
        /// Writes a byte to the current tar archive entry. This method simply calls Write(byte[],
        /// int, int).
        /// </summary>
        ...
        /// <summary>
        /// Writes bytes to the current tar archive entry. This method is aware of the current entry
        /// and will throw an exception if you attempt to write bytes past the length specified for
        /// the current entry. The method is also (painfully) aware of the record buffering required
        /// by TarBuffer, and manages buffers that are not a multiple of recordsize in length,
        ...
        public override void Write(byte[] buffer, int offset, int count)
        {
            ...
            // We have to deal with assembly!!! The programmer can be writing little 32 byte chunks
            // for all we know, and we must assemble complete blocks for writing. TODO REVIEW Maybe
            // this should be in TarBuffer? Could that help to eliminate some of the buffer copying.
            if (assemblyBufferLength > 0)
            {
            ...
            // When we get here we have EITHER: o An empty "assembly" buffer. o No bytes to write
            // (count == 0)
            while (count > 0)
            {
