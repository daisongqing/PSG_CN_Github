...
{
    /// <summary>
    /// This class represents an entry in a Tar archive. It consists of the entry's header, as well
    /// as the entry's File. Entries can be instantiated in one of three ways, depending on how they
    /// are to be used.
    /// <p>
    /// TarEntries that are created from the header bytes read from an archive are instantiated with
    /// the TarEntry( byte[] ) constructor. These entries will be used when extracting from or
    /// listing the contents of an archive. These entries have their header filled in using the
    /// header bytes. They also set the File to null, since they reference an archive entry not a file.
    /// </p>
    /// <p>
    /// TarEntries that are created from files that are to be written into an archive are
    /// instantiated with the CreateEntryFromFile(string) pseudo constructor. These entries have
    /// their header filled in using the File's information. They also keep a reference to the File
    /// for convenience when writing entries.
    /// </p>
    /// <p>
    /// Finally, TarEntries can be constructed from nothing but a name. This allows the programmer to
    /// construct the entry by hand, for instance when only an InputStream is available for writing
    /// to the archive, and the header information is constructed from other information. In this
    /// case the header fields are set to defaults and the File is set to null.
    /// </p>
    /// <see cref="TarHeader"/>
    /// </summary>
    public class TarEntry : ICloneable
    {
        ...
        /// <summary>
        /// Construct an entry with only a <paramref name="name"> name </paramref>. This allows the
        /// programmer to construct the entry's header "by hand".
        /// </summary>
        ...
        /// <summary>
        /// Determine if the given entry is a descendant of this entry. Descendancy is determined by
        /// the name of the descendant starting with this entry's name.
        /// </summary>
        ...
        public void GetFileTarHeader(TarHeader header, string file)
        {
            ...
            // 23-Jan-2004 GnuTar allows device names in path where the name is not local to the
            // current directory
            ...
            /*
                        ...
                        {
                            // check if the OS is Windows Strip off drive letters!
                            if (name.Length > 2)
                            {
                                char ch1 = name[0];
                                char ch2 = name[1];
            ...
            */
            ...
            // No absolute pathnames Windows (and Posix?) paths can start with UNC style
        ...
        /// <summary>
        /// Get entries for all files present in this entries directory. If this entry doesnt
        /// represent a directory zero entries are returned.
        /// </summary>
        ...
        /// <summary>
        /// Convenience method that will modify an entry's name directly in place in an entry header
        /// buffer byte array.
        /// </summary>
        /// <param name="buffer">  The buffer containing the entry header to modify. </param>
