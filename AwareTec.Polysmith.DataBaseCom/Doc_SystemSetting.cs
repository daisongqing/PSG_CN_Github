...
namespace AwareTec.Polysmith.DataBaseCom
{
    ...
    /// 系统配置
    /// </summary>
    public class Doc_SystemSetting : ITable, ICloneable
    {
        public Doc_SystemSetting() { }
        public Doc_SystemSetting(bool init)
        {
            if (init)
            {
                this.SaveEdfPath = "...\\Data";
                this.ReportPath = Environment.GetFolderPath(Environment.SpecialFolder.DesktopDirectory) + "\\pSleepReport";
                this.ReportType = "Pdf";
                this.PlaySpanTime = 1;
                this.AutoRunReport = true;
                this.AllowRemoteAnalysis = false;
                this.VedioSavePath = "...\\Media";
                this.OxygenReduceRange = "3";
                this.AnalysisBeforeReport = true;
                this.ReportChartCanSelected = false;
                this.UseBirthdayEnable = false;
                this.Reserve1 = "40";
                this.Reserve2 = "0";
                this.NotSplitRecordFile = false;
                this.MaxRecodFileLenght = 300;
            }
        }
        #region 私有成员
        private int m_ID = 0;
        ...
        private bool m_NotSplitRecordFile_ini = false;
        private int m_MaxRecodFileLenght = 300;
        private bool m_MaxRecodFileLenght_ini = false;
        private string m_Reserve1 = "40";
        ...
        public string SaveEdfPath
        {
            set
            {
                m_SaveEdfPath = value.Replace(AppDomain.CurrentDomain.BaseDirectory, "...\\");
                m_SaveEdfPath_ini = true;
            }
            get
            {
                return m_SaveEdfPath.Replace("...\\", AppDomain.CurrentDomain.BaseDirectory);
            }
        }
        ...
        public string ReportTemplate
        {
            set
            {
                m_ReportTemplate = value.Replace(AppDomain.CurrentDomain.BaseDirectory, "...\\");
                m_ReportTemplate_ini = true;
            }
            get
            {
                return m_ReportTemplate.Replace("...\\", AppDomain.CurrentDomain.BaseDirectory);
            }
        }
        public string ReportTemplate2
        {
            set
            {
                m_ReportTemplate2 = value.Replace(AppDomain.CurrentDomain.BaseDirectory, "...\\");
                m_ReportTemplate2_ini = true;
            }
            get
            {
                return m_ReportTemplate2.Replace("...\\", AppDomain.CurrentDomain.BaseDirectory);
            }
        }
        ...
        public string VedioSavePath
        {
            set
            {
                m_VedioSavePath = value.Replace(AppDomain.CurrentDomain.BaseDirectory, "...\\");
                m_VedioSavePath_ini = true;
            }
            get
            {
                return m_VedioSavePath.Replace("...\\", AppDomain.CurrentDomain.BaseDirectory);
            }
        }
        ...
        public int MaxRecodFileLenght
        {
            ...
            {
                if (value == 0)
                    m_MaxRecodFileLenght = 300;
                else
                    m_MaxRecodFileLenght = value;
                m_MaxRecodFileLenght_ini = true;
            }
            get
            {
                return m_MaxRecodFileLenght;
            }
        }
        /// <summary>
        /// 暂时用来存放氧减报警的最小阈值
        ...
        public string GetInsertString()
        {
            ...
            if (this.m_NotSplitRecordFile_ini)
            {
                def = string.Format("{0}{2}{1}", def, "NotSplitRecordFile", Mulitcase ? "," : "");
                val = string.Format("{0}{2}{1}", val, m_NotSplitRecordFile ? 1 : 0, Mulitcase ? "," : "");
                Mulitcase = true;
            }
            if (this.m_MaxRecodFileLenght_ini)
            {
                def = string.Format("{0}{2}{1}", def, "MaxRecodFileLenght", Mulitcase ? "," : "");
                val = string.Format("{0}{2}{1}", val, m_MaxRecodFileLenght, Mulitcase ? "," : "");
                Mulitcase = true;
            }
        ...
        public string GetSelectString()
        {
            ...
            if (m_NotSplitRecordFile_ini)
            {
                returnstr = string.Format("{0}NotSplitRecordFile={1}", Mulitcase ? string.Format("{0} AND ", returnstr) : "", m_NotSplitRecordFile ? 1 : 0);
                Mulitcase = true;
            }
            if (m_MaxRecodFileLenght_ini)
            {
                returnstr = string.Format("{0}MaxRecodFileLenght={1}", Mulitcase ? string.Format("{0} AND ", returnstr) : "", m_MaxRecodFileLenght);
                Mulitcase = true;
            }
            if (m_Reserve1_ini)
            {
        ...
        public string GetUpdateString()
        {
            ...
            if (m_NotSplitRecordFile_ini)
            {
                def = string.Format("{0}{2}NotSplitRecordFile={1}", def, m_NotSplitRecordFile ? 1 : 0, Mulitcase ? "," : "");
                Mulitcase = true;
            }
            if (m_MaxRecodFileLenght_ini)
            {
                def = string.Format("{0}{2}MaxRecodFileLenght={1}", def, m_MaxRecodFileLenght, Mulitcase ? "," : "");
                Mulitcase = true;
            }
            if (m_Reserve1_ini)
            {
        ...
        public void SetValue(string strName, object Value)
        {
            switch (strName)
            {
                ...
                case "OxygenReduceRange":
                    ...
                    break;
                case "NotSplitRecordFile":
                    NotSplitRecordFile = Convert.ToBoolean(Value);
                    break;
                case "MaxRecodFileLenght":
                    MaxRecodFileLenght = Convert.ToInt32(Value);
                    break;
                case "Reserve1":
        ...
        public object Clone()
        {
            ...
            {
                UserID = this.UserID,
                SaveEdfPath = this.SaveEdfPath,
                ReportPath = this.ReportPath,
                ReportTemplate = this.ReportTemplate,
                ReportTemplate2 = this.ReportTemplate2,
                ReportType = this.ReportType,
                AutoRunReport = this.AutoRunReport,
                PlaySpanTime = this.PlaySpanTime,
                AllowRemoteAnalysis = this.AllowRemoteAnalysis,
                RemoteServerAddr = this.RemoteServerAddr,
                Password = this.Password,
                VedioSourceUrl = this.VedioSourceUrl,
                VedioSavePath = this.VedioSavePath,
                UseBirthdayEnable = this.UseBirthdayEnable,
                AnalysisBeforeReport = this.AnalysisBeforeReport,
                ReportChartCanSelected = this.ReportChartCanSelected,
                OxygenReduceRange = this.OxygenReduceRange,
                NotSplitRecordFile = this.NotSplitRecordFile,
                MaxRecodFileLenght = this.MaxRecodFileLenght,
