...
namespace AwareTec.Polysmith.Cloud.UI.Views
{
    public partial class HistoryDataView : UserControl
    {
        ...
        public pChart.Chart ChartArea = null;
        private pChart.ReportChart reportChart1 = null;
        private MyConfiguration m_myConfiguration = null;
        internal DataManger dataManager = null;
        private HScrollBarEx m_HScrollBarEx1 = null;
        ...
        public HistoryDataView()
        {
            ...
            ChartArea = new pChart.Chart(Color.FromArgb(239, 255, 253), Color.FromArgb(239, 255, 253));
            ChartArea.Dock = DockStyle.Fill;
            reportChart1 = new pChart.ReportChart(Color.FromArgb(239, 255, 253));
            reportChart1.Dock = DockStyle.Fill;
            m_HScrollBarEx1 = new HScrollBarEx
            {
                Dock = DockStyle.Fill
            };
            m_HScrollBarEx1.ValueChanged += m_HScrollBarEx1_ValueChanged;
            panel2.Controls.Add(m_HScrollBarEx1);
            splitContainerEx1.Panel1.Controls.Add(ChartArea);
            splitContainerEx1.Panel1.Controls.Add(panel2);
            panel3.Controls.Add(reportChart1);
            ...
            this.button1.Click += subMenubutton_Click;
            this.button2.Click += subMenubutton_Click;
            this.button3.Click += subMenubutton_Click;
            this.button4.Click += subMenubutton_Click;
            this.button5.Click += subMenubutton_Click;
            this.button6.Click += subMenubutton_Click;
            this.button7.Click += subMenubutton_Click;
            this.button8.Click += subMenubutton_Click;
            this.button9.Click += subMenubutton_Click;
            this.button10.Click += subMenubutton_Click;
            this.button11.Click += subMenubutton_Click;
            this.button12.Click += subMenubutton_Click;
            this.button13.Click += subMenubutton_Click;
            this.button14.Click += subMenubutton_Click;
        ...
        {
            AddMultSleep addsleep = new AddMultSleep();
            ...
            Size panelsize = this.splitContainerEx1.Panel1.Size;
        ...
        private void HistoryDataView_Load(object sender, EventArgs e)
        {
            ...
            {
                AddLog(string.Format("进入回放页面 根据Guid查找记录出错 {0}", ee), pSystem.LogManagement.LogLevel.ERROR);
            }
            //todo
        ...
        public bool mainKeyDown(KeyEventArgs e)
        {
            ...
            {
                PageMove(e.KeyCode == Keys.Up ? 0 : e.KeyCode == Keys.Down ? 3 : e.KeyCode == Keys.Left ? 1 : 2, !e.Alt);
                return true;
            }

            else if (e.Control && e.KeyCode == Keys.F && e.Shift)
            {
                HistoryDataView_KeyDown(null);
                return true;
            }
            else if (e.Control && e.KeyCode == Keys.F && !e.Shift)
            {
                HistoryDataView_KeyDown();
                return true;
            }
                ...
                || e.KeyCode == Keys.NumPad1 || e.KeyCode == Keys.NumPad2 || e.KeyCode == Keys.NumPad3 || e.KeyCode == Keys.NumPad5)
            {
                ...
                {
                    MarkSleep((e.KeyCode == Keys.W || e.KeyCode == Keys.NumPad0) ? 5 : (e.KeyCode == Keys.D1 || e.KeyCode == Keys.NumPad1) ? 3 : (e.KeyCode == Keys.D2 || e.KeyCode == Keys.NumPad2) ? 2 : (e.KeyCode == Keys.D3 || e.KeyCode == Keys.NumPad3) ? 1 : 4);
                }
                else
                {
        ...
        private void HistoryDataView_KeyDown(object sender)
        {
        ...
        /// 
        /// </summary>
        private void HistoryDataView_KeyDown()
        {
        ...
        private void HistoryDataView_MouseWheel(object sender, MouseEventArgs e)
        {
            ...
            else
            {
                ...
                DataTable currentChannelTable = GlobalSingleton.Instance.User.CurrentChannelConfig.CurrentDataTable;
                DataRow[] array = currentChannelTable.Select(string.Format("ChannelID='{0}'", curveItem.ID));
                if (array.Length <= 0)
                {
                    return;
                }
                ChannelTable doc_Channel = GlobalSingleton.Instance.User.CurrentChannelConfig.CurrentChannelTable.Find(t => t.ChannelID == curveItem.ID);
                ...
                for (int i = 0; i < intPixelRangArray.Length; i++)
                {
                    if (pixelRate == (float)intPixelRangArray[i])
                    {
                        ...
                        doc_Channel.Sensitivity = curveItem.PixelRate;
                        break;
                    }
                }
        ...
        {
            List<PredefinedCalibration> CalibrationDefinelist = GlobalSingleton.Instance.PredefinedData.PredefinedCalibrations; ;
            ...
            for (int index = CalibrationValues.Count - 1; index >= 0; index--)
            {
                if (dic_cailbrationvalue.Count < CalibrationDefinelist.Count)
                {
                    ...
                    else
                    {
                        ...
                        PredefinedCalibration PreDefinedCalibration = CalibrationDefinelist.Find((PredefinedCalibration t) => t.Id == realvalue);
                        if (PreDefinedCalibration != null)
                        {
                            string[] array3 = PreDefinedCalibration.ChannelID.Split('/');
                            for (int j = 0; j < array3.Length; j++)
                            {
                                ...
                                rectangleMarkers.Name = PreDefinedCalibration.Name;
        ...
        private void subMenubutton_Click(object sender, EventArgs e)
        {
            ...
            if (this.bak_butt != null)
            {
                ...
                this.bak_butt.NormalColor = Color.White;
            }
            button.NormalColor = Color.FromArgb(217, 250, 245);
        ...
        {
            timer1.Interval = m_userOperationConfig.AutomaticScrollingInterval * 1000;
            ...
            if (bak_butt.Tag != null)
            {
                ...
                switch (Convert.ToInt32(bak_butt.Tag))
                {
                    ...
                    case 4:
                        ...
                        break;
                    case 5:
                        data = ResultDomain.Default.SleepStagPoints;
                        break;
                    case 6:
                        data = ResultDomain.Default.BloodOxygenPoints;
                        break;
                    case 8:
        ...
        private bool ChartArea_DrawImageBeforeHandle()
        {
            ...
            {
                List<ChannelTable> channelTables = GlobalSingleton.Instance.User.CurrentChannelConfig.CurrentChannelTable;
                channelTables = (channelTables.Count == 0) ? GlobalSingleton.Instance.User.DefaultChannelConfig.CurrentChannelTable : channelTables;
                m_HScrollBarEx1.BaseTimeLine = ChartArea.BaseTimeLineSpan = Channel.Default.BaseTimeLineSpan;
                m_HScrollBarEx1.MoveOffValue = ChartArea.MoveTimeSpan;
                List<string> itemIDs = ChartArea.getItemIDs();
                for (int i = 0; i < channelTables.Count; i++)
                {
                    ChannelTable channelTable = channelTables[i];
                    CurveItem curveItem = ChartArea.FindCurve(channelTable.ChannelID);
                    if (curveItem == null)
                    {
                        curveItem = Channel.Default.CreatChannel(channelTables[i]);
                        if (curveItem.IsCloneItem)
                        {
                            pChart.CurveItem clonechannel = ChartArea.FindCurve(curveItem.ChannelNum, false);
                            if (clonechannel != null)
                            {
                                curveItem.CloneDataSource(clonechannel);
                            }
                        }
                        else if (channelTable.ID.ToString().Contains("Append_"))
                        {
                            string[] ids2 = channelTable.ID.ToString().Replace("Append_", "").Split(':');
                            if (ids2.Length == 2)
                            {
                    ...
                    curveItem.Name = channelTable.strName;
                    curveItem.Visible = channelTable.State;
                    curveItem.ChannelNo = channelTable.Index;
                    ...
                    curveItem.PixelRate = channelTable.Sensitivity;
                    curveItem.TimeSpan = 1000 / channelTable.SpanTime;
                    curveItem.PenColor = channelTable.ColorSelect;
                    ...
                    curveItem.belong = Convert.ToInt32(channelTable.Reserve.Split(':')[0]);
                    ...
                    itemIDs.Remove(channelTable.ChannelID);
                }
        ...
        private bool ChartArea_ReadChannelDataExHandler(DateTime start, DateTime end, List<CurveItem> curveItems)
        {
            ...
            for (int i = 0; i < curveItems.Count; i++)
            {
                ...
                if (item.IsCloneItem)
                {
                    CurveItem curveItem2 = ChartArea.FindCurve(item.ChannelNum,false);
                    if (curveItem2 != null)
                    {
        ...
        private bool ChartArea_MarkTypeChanged(IMarker[] marks, pChart.IMarker.MarkType NewMarkTyp, string NewMarkName)
        {
            if (Channel.Default.ScoreLock)
            {
                AhDung.MessageTip.ShowWarning("评分已结束，如需重新评分，请先解锁评分状态");
                return false;
            }
            if (marks.Length == 0)
                return false;
            ...
            if (find != null)
            {
                if (find is RectangleMarkers)
                {
                    ...
                    if (string.Join("/", (find as RectangleMarkers).OnChannelIndexs) != string.Join("/", (marks[0] as RectangleMarkers).OnChannelIndexs))
                    {
                        this.Invoke(new MethodInvoker(() =>
                        {
                            AhDung.MessageTip.ShowWarning("将更改的事件不适用所选通道！");
                        }));
                    ...
                    if ((find as RectangleMarkers).MinLimitValue > (marks[0] as RectangleMarkers).MinLimitValue)
                    {
                        this.Invoke(new MethodInvoker(() =>
                        {
                            AhDung.MessageTip.ShowWarning(string.Format("更改失败：当前事件持续时间<{0}s！", (find as RectangleMarkers).MinLimitValue));
                        }));
                ...
                AddLog(string.Format("更改区域标签事件,事件类型为 {0}，事件名称为 {1}，事件开始时间为 {2}，事件结束时间为 {3} ", oneMark.MarkTyp, oneMark.Name, oneMark.StartTime, oneMark.EndTime), pSystem.LogManagement.LogLevel.DEBUG);
        ...
        private bool ChartArea_SelectRectangleKeyDownHandler(string ChannelID, CurveItem item)
        {
            ...
            if (marker != null)
            {
                ...
                {
                    if ((marker as pChart.RectangleMarkers).OnChannelIndexs.Contains(GlobalSingleton.Instance.User.CurrentChannelConfig.CurrentChannelTable.Find(t => t.ChannelID == ChannelID).ID))
                    //if ((marker as pChart.RectangleMarkers).OnChannelIndexs.Contains(Channel.Default.ChannelProperties.Find(t => t.ID.ToString() == ChannelID).ID))
                    {
                        (mark as pChart.RectangleMarkers).MinLimitValue = (marker as pChart.RectangleMarkers).MinLimitValue;
                    }
                    else
                    {
                        AhDung.MessageTip.ShowWarning("事件类型不匹配！");
                        return false;
                    }
            ...
            if (mark.Delete)
            {
                Task.Factory.StartNew(delegate
                {
                    if (mark.MarkTyp == IMarker.MarkType.LightOff || mark.MarkTyp == IMarker.MarkType.LightOn)
                    {
                        ...
                        AddLog(string.Format("删除开关灯事件，删除的事件类型{0}，事件时间为{1}", mark.MarkTyp.ToString(), mark.MarkCreatTime), pSystem.LogManagement.LogLevel.DEBUG);
        ...
        {
            if (channelID.Contains("Clone_"))
            {
                AhDung.MessageTip.ShowWarning("派生通道不能做标记", -1, null, null, false);
                return false;
            }
                ...
                if (mark.MarkTyp == IMarker.MarkType.LightOn)
                {
                    ...
                    if (marker != null)
                    {
                        if (marker.ID != mark.ID)///开灯是新创建或者由老的开灯变时
                        {
                            ...
                            {
                                AhDung.MessageTip.ShowWarning("标记开灯的时间不能早于关灯时间", -1, null, null, false);
                                return false;
                            }
                            else if ((mark.MarkCreatTime - marker.MarkCreatTime).TotalMinutes < 1)
                            {
                                AhDung.MessageTip.ShowWarning("开关灯时间间隔不能小于1min", -1, null, null, false);
                                return false;
                            }
                        }
                ...
                else if (mark.MarkTyp == IMarker.MarkType.LightOff)
                {
                    ...
                    if (marker2 != null)
                    {
                        if (marker2.ID != mark.ID)
                        {
                            ...
                            {
                                AhDung.MessageTip.ShowWarning("标记关灯的时间不能迟于开灯时间", -1, null, null, false);
                                return false;
                            }
                ...
                if (find == null || issystemevent == false)
                {
                    if (currentIDChanged)
                    {
                        ...
                        this.ChartArea.pChartMarks.RemoveAll(t => t.MarkTyp == oldTyp);
                        {
                            ...
                            AddLog(string.Format("删除开关灯事件,事件类型为 {0}，事件时间为{1}", oldTyp.ToString(), oldimark.MarkCreatTime), pSystem.LogManagement.LogLevel.DEBUG);
                        }
                    }
                    mark.ID = markID;
                    ...
                    AddLog(string.Format("新增点位标签事件,事件名称为 {0}，事件时间为{1}", mark.Name, mark.MarkCreatTime), pSystem.LogManagement.LogLevel.DEBUG);
                }
                else
                {
                    ...
                    if (!currentIDChanged)///如果之前存在同类型事件
                    {
                        ...
                        if (oldTyp != mark.MarkTyp)///判断mark更改前的事件类型是否与更改后的事件类型一致，不一致则需要删除mark
                        {
                            ...
                            this.ChartArea.pChartMarks.RemoveAll(t => t.MarkTyp == oldTyp && t.ID == mark.ID);
                            {
                                ...
                                AddLog(string.Format("删除开关灯事件,事件类型为 {0}，事件时间为{1}", oldTyp.ToString(), oldimarkertime), pSystem.LogManagement.LogLevel.DEBUG);
                            }
                        }
                        dataManager.dataFactory.EventsInstance.Add(new DataMangement.EventRecordsUit.EventRecord()
                        {
                        ...
                        AddLog(string.Format("更新开关灯事件,事件类型为 {0}，事件时间为{1}", mark.MarkTyp.ToString(), mark.MarkCreatTime), pSystem.LogManagement.LogLevel.DEBUG);
                    }
                    else
                    {
                        dataManager.dataFactory.EventsInstance.Add(new DataMangement.EventRecordsUit.EventRecord()
            ...
            else
            {
                        ...
                        else
                        {
                            ...
                            if (mm.MarkTyp != mark.MarkTyp)
                            {
                                ...
                                if (isfirst)
                                {
                                    ...
                                    AddLog(string.Format("更新区域标签事件,旧事件类型为 {0}，新事件类型为 {3},新事件开始时间为 {1}，新事件结束时间为 {2} ", mm.MarkTyp.ToString(), (mark as RectangleMarkers).StartTime, (mark as RectangleMarkers).EndTime, mark.MarkTyp.ToString()), pSystem.LogManagement.LogLevel.DEBUG);
                                }
                ...
                if (isadd)
                {
                    ...
                    AddLog(string.Format("新增区域标签事件,事件所在通道为 {0}，事件名称为 {1}，事件开始时间为 {2}，事件结束时间为 {3} ", channelID, rectangleMarkers.Name, rectangleMarkers.StartTime, rectangleMarkers.EndTime), pSystem.LogManagement.LogLevel.DEBUG);
        ...
        private void changeMarkRange(IMarker bechangedmark, List<IMarker> removeMarks, bool needFresh = true)
        {
            if (bechangedmark is RectangleMarkers)
            {
                ...
                if (matchOldMarkID != "")
                {
                    ...
                    AddLog(string.Format("更新区域标签事件,事件类型为 {0}，事件开始时间为 {1}，事件结束时间为 {2} ", mark.MarkTyp.ToString(), mark.StartTime, mark.EndTime), pSystem.LogManagement.LogLevel.DEBUG);
        ...
        private void deleteMarks(string channelID, List<IMarker> marks, bool refrshEventList = true, bool realdeleteevent = true)
        {
            for (int i = 0; i < marks.Count; i++)
            {
                ...
                if (mark != null)
                {
                    ...
                    AddLog(string.Format("删除区域标签事件，事件类型为{0}，事件开始时间{1}，事件结束时间{2}", mark.MarkTyp.ToString(), mark.StartTime, mark.EndTime), pSystem.LogManagement.LogLevel.DEBUG);
                ...
                if (mark2 != null)
                {
                    ...
                    AddLog(string.Format("删除点位标签事件，删除的事件名称为{0}，事件时间为{1}", mark2.Name, mark2.MarkCreatTime), pSystem.LogManagement.LogLevel.DEBUG);
        ...
        private void CheckPLMS(RectangleMarkers mark, int timespan, bool add = true)
        {
            if (mark.MarkTyp == pChart.IMarker.MarkType.LegMove)
            {
                ...
                if (find.Count > 1)
                {
                    ...
                    RectangleMarkers modle = MarkerManage.Default.DefineMarkers.Find(t => t.MarkTyp == pChart.IMarker.MarkType.PeriodicalBodyMove) as RectangleMarkers;
        ...
        private void UpdateArousalScore(IMarker oldMarker, IMarker.MarkType newMarkTyp)
        {
            ...
            if (respArousal != null)
            {
                ...
                AddLog(string.Format("更新区域标签事件,事件类型为 {0}，事件开始时间为 {1}，事件结束时间为 {2} ", newMarkTyp.ToString(), (oldMarker as RectangleMarkers).StartTime, (oldMarker as RectangleMarkers).EndTime), pSystem.LogManagement.LogLevel.DEBUG);
            }
        }
        /// <summary>
        /// 根据事件类型找关联微觉醒标记
        ...
        private void ZoomView_Click(object sender, EventArgs e)
        {
            ...
            CloudSkinForm skinForm = new CloudSkinForm();
            skinForm.Text = "局部放大";
        ...
        {
            ChannelTable channelTable = GlobalSingleton.Instance.User.CurrentChannelConfig.CurrentChannelTable.Find((ChannelTable t) => t.ID == channel.ID);
            ...
            curveItem.Name = channel.strName;
            curveItem.PixelRate = (channel.PixelEnable ? channel.Sensitivity : 0f);
            ...
            curveItem.PenColor = channel.ColorSelect;
        ...
        public EDF Bind(string fileName, bool isfirst = false)
        {
            ...
            if (m_EdfInstance.DataSource.IsCorrect)
            {
                ...
                //给属性赋值
                Channel.Default.StartTime = m_Start;
                Channel.Default.EndTime = m_end;
                TimeSpan span = m_end - m_Start;
                int totalSec = (int)span.TotalSeconds;
                m_TotalFrameCnt = ((totalSec % 30 == 0) ? (totalSec / 30) : (totalSec / 30 + 1));
                dataManager.Initialize(GlobalSingleton.Instance.getSystemSetting().OrderPath, GUID, TotalFrameCnt);
                ...
                //ChartArea.MarkEnable = Channel.Default.ScoreLock;
                GC.Collect();
        ...
        public void MarkSleep(int typ, bool iskeyDown = false)
        {
            ...
            if (num < ResultDomain.Default.SleepStagPoints.Length && num >= 0)
            {
                ...
                Channel.Default.AnalysisReult.HasDataChange = true;
                AddLog(string.Format("用户手动标记睡眠分期，当前帧序号为{0}，标记的睡眠分期为{1}", this.m_HScrollBarEx1.CurrentFrameCnt - 1, this.ChartArea.SleepStage[num]), pSystem.LogManagement.LogLevel.DEBUG);
            }
        ...
        /// 加载之前分析的轨迹
        /// </summary>
        public void LoadAnalysisData(AnalysisResult data, bool isanalysis = false)
        {
            if (data != null)
            {
                ...
                data.StartTime = m_Start;
                data.EndTime = m_end;
                Channel.Default.AnalysisReult = data;
        ...
        /// </summary>
        /// <returns></returns>
        public AddOrEditMonitoringDataRequestModel getRemoteAnalysisResult(Doc_DataResult result)
        {
            AddOrEditMonitoringDataRequestModel ret = new AddOrEditMonitoringDataRequestModel();
            ret.eventMarkers = dataManager.dataFactory.EventsInstance.ToString();
            ret.frameInfo= dataManager.dataFactory.EpochsInstance.ToString();
            ret.spO2Info = dataManager.dataFactory.Spo2Instance.ToString(Spo2Values);
            ret.analysisResults = dataManager.dataFactory.ResultInstance.ToString(result);
            ret.orderId = result.GUID;
            ret.calibrationInfo = "";
            ret.version = _orderPath.Version;
            return ret;
        }
        /// <summary>
        /// 主动释放资源
            ...
            if (EventRecordCase.SelectedItem != null)
            {
                ...
                if (!string.IsNullOrEmpty(selectevent))
                {
                    if (selectevent == "氧减")
                    {
                        ...
                        toolStripComboBox1.Items.Add("最小血氧值");
                        toolStripComboBox1.Items.Add("最长氧减");
                    }
                    else if (selectevent == "低通气")
                    {
                        toolStripComboBox1.Items.Clear();
                        toolStripComboBox1.Visible = true;
                        toolStripComboBox1.Items.Add(" ");
                        toolStripComboBox1.Items.Add("最长低通气");
                    }
                    else if (selectevent == "混合性呼吸暂停")
                    {
                        toolStripComboBox1.Items.Clear();
                        toolStripComboBox1.Visible = true;
                        toolStripComboBox1.Items.Add(" ");
                        toolStripComboBox1.Items.Add("最长混合性呼吸暂停");
                    }
                    else if (selectevent == "阻塞性呼吸暂停")
                    {
                        toolStripComboBox1.Items.Clear();
                        toolStripComboBox1.Visible = true;
                        toolStripComboBox1.Items.Add(" ");
                        toolStripComboBox1.Items.Add("最长阻塞性呼吸暂停");
                    }
                    else if (selectevent == "中枢性呼吸暂停")
                    {
                        toolStripComboBox1.Items.Clear();
                        toolStripComboBox1.Visible = true;
                        toolStripComboBox1.Items.Add(" ");
                        toolStripComboBox1.Items.Add("最长中枢性呼吸暂停");
                    }
                    else if (selectevent == "周期性循环腿动")
                    {
                        toolStripComboBox1.Items.Clear();
                        toolStripComboBox1.Visible = true;
                        toolStripComboBox1.Items.Add(" ");
                        toolStripComboBox1.Items.Add("相关联腿动");
                    }
                    else
                    {
        ...
        private void FillEventData(int markTyp = -1)
        {
            ...
            if (bak_EventRecordList.Count != 0)
            {
                ...
                PLMsEvents = bak_EventRecordList.FindAll(t => t.MarkTyp == IMarker.MarkType.PeriodicalBodyMove);
                try
                {
            ...
            for (int i = 0; i < select.Count; i++)
            {
                ...
                {
                    source.Rows.Add(mark.MarkCreatTime, 1, mark.Name, mark.Description, "点位标签", mark.StartFrameNo, mark.StartFrameNo, mark.ID);
                }
                else
                {
                    ...
                    source.Rows.Add(mark.MarkCreatTime, Math.Round((mark2.EndTime - mark2.StartTime).TotalSeconds, 2), mark.Name, des, "区域标签", mark.StartFrameNo, mark.EndFrameNo, mark.ID);
                }
            }
        ...
        private void initSelectText()
        {
            ...
            {
                IMarker mark = MarkerManage.Default.DefineMarkers[i];
                if (mark.MarkTyp != IMarker.MarkType.None)
                {
                    if (mark is RectangleMarkers)
                    {
        ...
        public AnalysisResult getBaseInfoer(string guid)
        {
            ...
            for (int i = 0; i < m_TotalFrameCnt; i++)
            {
                ...
                if (list.Count() > 0)
                    one.HeartRate = string.Format("{0},{1},{2:f2}", list.Min(), list.Max(), list.Average());
                else
                    one.HeartRate = "0,0,0";
        ...
        private void toolStripComboBox1_SelectedIndexChanged(object sender, EventArgs e)
        {
            if (selectgridview.Rows.Count > 0)
            {
                ...
                else if (toolStripComboBox1.SelectedItem.ToString() == "最小血氧值" && isfirstselect == false)
                {
                    double minspo2 = 101;
                    int minspo2index = 0;
                ...
                else if (toolStripComboBox1.SelectedItem.ToString() == "相关联腿动" && isfirstselect == false)
                {
                    int PLMsLegsCount = 0;
                    RectangleMarkers rectangleMarkers = new RectangleMarkers();
                    ...
                    for (int i = 0; i < bak_legsevents.Count; i++)
                    {
                        for (int k = 0; k < baksource.Rows.Count; k++)
                        {
                            ...
                            {
                                AddLog(string.Format("点击相关联腿动出错，错误信息为：{0}", ee.Message), pSystem.LogManagement.LogLevel.ERROR);
                            }
                        }
                    }
                    //在第一行增加总次数，以记录开始时间为创建时间，帧固定为第一帧，确保排序后一定在第一行
                    newsource.Rows.Add(m_Start, 0, "PLMs总次数", string.Format("{0}次", PLMsLegsCount), "区域标签", 1, 1, "");
                    //按照事件创建时间排序
