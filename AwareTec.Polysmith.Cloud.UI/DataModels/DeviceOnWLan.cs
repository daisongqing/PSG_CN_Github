   ...
   public class DeviceOnWLan
    {
       ...
       public DeviceOnWLan()
       {
        ...
        private void m_server_ClientDown(IOClient client)
        {
            ...
            DataModels.Global.GlobalSingleton.Instance.LogInstance.AddLog(string.Format("远程连接掉线 {0}",showlog),pSystem.LogManagement.LogLevel.ERROR);
        }

        private void m_server_RecConnetHandle(IOClient client)
        {
            DataModels.Global.GlobalSingleton.Instance.LogInstance.AddLog(string.Format("接收到客户端连接(matchkey:{0})", client.MatchKey), pSystem.LogManagement.LogLevel.WARN);
            IDevice find = null;
            ...
            if (find == null)
            {
                ...
                server.getBlueToothName();
                DataModels.Global.GlobalSingleton.Instance.LogInstance.AddLog(string.Format("客户端连接(matchkey:{0})添加到队列", client.MatchKey), pSystem.LogManagement.LogLevel.WARN);
            }
            else
            {
        ...
        {
            DataModels.Global.GlobalSingleton.Instance.LogInstance.AddLog(string.Format("成功识别客户端连接(matchkey:{0})，其对应设备名称为{1}", matchKey, devName), pSystem.LogManagement.LogLevel.WARN);
            ...
            if (find != null)
            {
                ...
                if (CurrentDevice != null)
                    if (find.DeviceName == CurrentDevice.DeviceName)///触发重连机制
                    {
                        ...
                        DataModels.Global.GlobalSingleton.Instance.LogInstance.AddLog(string.Format("设备重连 重连的设备名称为{0}", find.DeviceName), pSystem.LogManagement.LogLevel.WARN);
                    }
            }
