...
namespace AwareTec.Polysmith.UI.FunctionControls
{
    public partial class RealDataView : UserControl
    {
        ...
        private void RealDataView_Load(object sender, EventArgs e)
        {
            ...
            hasvideo = m_hasVedio;
            if(m_hasVedio && Channel.Default.SystemSetting.Reserve2 == "1")
            {
        ...
        private void ChartArea_ShowOrHideChannelHandler(CurveItem curveItem)
        {
            ...
            if (dr.Length != 1)
                throw new Exception(Program.Language == "EN" ? "The data seems to have some errors" : "数据似乎有些错误");
        ...
        private void Defalut_DoWork(tools.ProgressTipForm sender, DoWorkEventArgs e)
        {
            ...
            {
                sender.SetError(string.Format(Program.Language == "EN" ? "Missing vc_dedist.x64 driver package：{0}" : "缺少vc_redist.x64 驱动包：{0}", ee.Message));
                goto getOut;
            }
            ...
            if (finddevs == null)
            {
                sender.SetError(Program.Language == "EN" ? "The current device is processing offline mode, please try again later!" : "当前设备处理离线模式，请稍后重试！");
                goto getOut;
            }
            ...
            {
                sender.SetError(string.Format(Program.Language == "EN" ? "Failed to connect device {0}" : "连接设备{0}失败", devName));
                goto getOut;
            }
            ...
            {
                sender.SetError(string.Format(Program.Language == "EN" ? "{0} initialization failed" : "{0}初始化失败", devName));
                goto getOut;
            }
            ///为了兼容不带定时功能的设备，暂时不判断
            ...
            if (m_Protocol.UserData.DeviceBusy)
            {
                ...
                {
                    sender.SetError(string.Format(Program.Language == "EN" ? "Failed to obtain case information for {0}" : "{0}的病例信息获取失败", devName));
                    goto getOut;
                }
                else
                {
                    if (m_MainViewRecord == null)
                    {
                        ...
                        if (m_MainViewRecord != null)
                        {
                            ...
                            if (ShowMessage(string.Format(Program.Language=="EN"? "Found cases that match the current device information({0} {1} {2})，Do you want to continue accessing monitoring？" : "已找到与当前设备信息匹配的历史病例({0} {1} {2})，是否要继续接入监测？", info.PatientNo, info.PatientName, m_MainViewRecord.RecordTime), containControl))
                            {
                                ...
                                {
                                    sender.SetError(Program.Language == "EN" ? "Failed to start listening!" : "启动监听失败!");
                                    goto goreturn;
                                }
                                if (sender.CancellationPending)
                                {
                            ...
                            {
                                sender.SetError(Program.Language == "EN" ? "Please press the OK button to end this operation!" : "请按确定键结束本次操作!");
                                goto goreturn;
                            }
                        }
                        else
            ...
            do
            {
                ...
                {
                    sender.SetError(Program.Language == "EN" ? "Case information writing failed: device unresponsive or PSG sleep monitoring in progress!" : "病例信息写入失败：设备无响应或者PSG睡眠监测中！");
                    goto getOut;
                }
                else
                {
            ...
            if(trycount>=5)
            {
                sender.SetError(Program.Language == "EN" ? "Case information writing failed: inconsistent matching code!" : "病例信息写入失败：匹配码不一致！");
            ...
            {
                sender.SetError(Program.Language == "EN" ? "Device information writing failed!" : "设备信息写入失败!");
                goto getOut;
            }
            if (sender.CancellationPending)
            {
        ...
        private float EstimateBloodPressure_spo2 = 98;
        private float EstimateBloodPressure_pr = 70;
        private int EstimateBloodPressure_Cnt = 0;
        private int EstimateBloodPressure_Sbp = 120;
        private int EstimateBloodPressure_Dbp = 80;

        private static readonly Random _random = new Random();

        // 定义血压结果结构体
        public struct BloodPressureResult
        {
            public int Sbp;
            public int Dbp;
        }
        ...
        /// <summary>
        /// 基于血氧饱和度和脉率估算血压（返回整数结果）
        /// </summary>
        static BloodPressureResult EstimateBloodPressure(float spo2, float pr)
        {
            // 正常基准值
            const float normalSBP = 120f;
            const float normalDBP = 80f;
            const float normalPR = 75f;
            const float normalSpO2 = 98f;

            // 计算脉率对血压的影响
            float prEffect = CalculatePREffect(pr, normalPR);

            // 计算血氧饱和度对血压的影响
            float spo2Effect = CalculateSpO2Effect(spo2, normalSpO2);

            // 计算总调整值
            float totalAdjustment = prEffect + spo2Effect;

            // 计算估算血压值（浮点数）
            float sbp = normalSBP + totalAdjustment;
            float dbp = normalDBP + (totalAdjustment * 0.6f); // 舒张压变化较小

            // 确保血压在合理范围内（使用优化边界处理）
            sbp = SmartClamp(sbp, 70f, 140f);
            dbp = SmartClamp(dbp, 40f, 90f);

            // 四舍五入为整数
            int sbpInt = (int)Math.Round(sbp);
            int dbpInt = (int)Math.Round(dbp);

            // 返回结果
            return new BloodPressureResult { Sbp = sbpInt, Dbp = dbpInt };
        }

        /// <summary>
        /// 应用小范围随机变化（模拟真实测量波动）
        /// </summary>
        static BloodPressureResult ApplyRandomVariation(BloodPressureResult result)
        {
            // 生成随机变化值（-3到+3 mmHg之间）
            int sbpVariation = _random.Next(-2, 3);
            int dbpVariation = _random.Next(-2, 3);

            // 应用变化
            int variedSbp = result.Sbp + sbpVariation;
            int variedDbp = result.Dbp + dbpVariation;

            // 确保变化后的值仍在合理范围内（使用优化边界处理）
            variedSbp = SmartClamp(variedSbp, 70, 140);
            variedDbp = SmartClamp(variedDbp, 40, 90);

            return new BloodPressureResult { Sbp = variedSbp, Dbp = variedDbp };
        }

        /// <summary>
        /// 计算脉率对血压的影响
        /// </summary>
        static float CalculatePREffect(float pr, float normalPR)
        {
            // 脉率差异
            float prDiff = pr - normalPR;
            ...
            {
                // 脉率增加时影响更大
                effect = 0.35f * prDiff + 0.002f * (float)Math.Pow(prDiff, 1.5f);
            }
            else if (prDiff < 0) // 脉率低于正常值
            {
                // 脉率减少时影响较小
                effect = 0.25f * prDiff;
            }
        ...
        /// 计算血氧饱和度对血压的影响
        /// </summary>
        static float CalculateSpO2Effect(float spo2, float normalSpO2)
        {
            // 血氧差异
            float spo2Diff = normalSpO2 - spo2;

            // 非线性影响函数
            float effect = 0f;

            if (spo2Diff > 0) // 血氧低于正常值
            {
                // 血氧降低时影响更大（指数关系）
                effect = 0.25f * spo2Diff + 0.01f * (float)Math.Pow(spo2Diff, 2f);
            }
            else if (spo2Diff < 0) // 血氧高于正常值
            {
                // 血氧升高时影响较小
                effect = 0.1f * spo2Diff;
            }
        ...
        /// <summary>
        /// 智能边界处理 - 当值超出范围时添加小随机数
        /// </summary>
        static float SmartClamp(float value, float min, float max)
        {
            if (value < min)
            {
                // 低于最小值时，在最小值附近添加小随机数
                float offset = (float)_random.NextDouble() * 3f; // 0-3 mmHg随机偏移
                return Math.Min(min + offset, max); // 确保不超过最大值
            }
            else if (value > max)
            {
                // 高于最大值时，在最大值附近减去小随机数
                float offset = (float)_random.NextDouble() * 3f; // 0-3 mmHg随机偏移
                return Math.Max(max - offset, min); // 确保不低于最小值
            }
            return value;
        }
        ...
        /// 智能边界处理（整数版本）
        /// </summary>
        static int SmartClamp(int value, int min, int max)
        {
            if (value < min)
            {
                // 低于最小值时，在最小值附近添加小随机数
                int offset = _random.Next(0, 4); // 0-3 mmHg随机偏移
                return Math.Min(min + offset, max); // 确保不超过最大值
            }
            else if (value > max)
            {
        ...
        private void ThermalMuzzleFlow_RecOneDataEventHandle(float data, bool Invaild, string msg)
        {
            ...
            if  (Program.Language == "EN" )
                 m_viewResult.ThermalBreathingRate = string.Format("Thermal Resp：{0}/min", xfChannel4.getBreathChannelState(data) ? value.ToString() : "--");
            else
        ...
        private void PressureMuzzleFlow_RecOneDataEventHandle(float data, bool Invaild, string msg)
        {
            ...
            if (Program.Language == "EN") { 
                m_viewResult.Pressure = string.Format("pressure：{0:f2}cmH2O", data / 98.067);//pa转厘米水柱
                m_viewResult.PressureBreathingRate = string.Format("pressure Resp：{0}/min", xfChannel3.getBreathChannelState(data) ? value.ToString() : "--");
            }else
            {
        ...
        private void AbdominalBreathing_RecOneDataEventHandle(float data, bool Invaild, string msg)
        {
            ...
            if (Program.Language == "EN")
                m_viewResult.AbdominalBreathingRate = string.Format("Abdominal Resp：{0}/min", xfChannel2.getBreathChannelState(data) ? value.ToString() : "--");
            else
        ...
        private void ChestBreathing_RecOneDataEventHandle(float data, bool Invaild, string msg)
        {
            ...
            if (Program.Language == "EN")
                m_viewResult.ChestBreathingRate = string.Format("Chest Resp：{0}/min", xfChannel1.getBreathChannelState(data) ? value.ToString() : "--");
            else
        ...
        private void PulseRate_RecOneDataEventHandle(float data, bool Invaild, string msg)
        {
            ...
            EstimateBloodPressure_pr = data;
            EstimateBloodPressure_Cnt = EstimateBloodPressure_Cnt + 1;

            if (Program.Language == "EN")
            {
                msg = "Module Fault";
                m_viewResult.PulseRate = string.Format("Pulse Rate:{0}", Invaild ? msg : string.Format("{0}/min", data));
            }
            else
            {
                if (true) { 
                     BloodPressureResult result = EstimateBloodPressure(EstimateBloodPressure_pr, EstimateBloodPressure_spo2);
                    
                    
                        //if (EstimateBloodPressure_Cnt > 4)
                          //  if ((EstimateBloodPressure_Cnt % 5) == 0)
                                m_viewResult.PulseRate = string.Format("血压:({0}/{1}) 脉率:{2}", Invaild ? "-" : string.Format("{0}", result.Sbp), Invaild ? "-" : string.Format("{0}", result.Dbp), Invaild ? msg : string.Format("{0}次/min", data));
                            //else
                             //   m_viewResult.PulseRate = string.Format("血压:({0}/{1}) 脉率:{2}", Invaild ? "-" : string.Format("{0}", EstimateBloodPressure_Sbp), Invaild ? "-" : string.Format("{0}", EstimateBloodPressure_Dbp), Invaild ? msg : string.Format("{0}次/min", data));
                    

                    EstimateBloodPressure_Sbp = result.Sbp;
                    EstimateBloodPressure_Dbp = result.Dbp;
                }
                else
                {
        ...
        private void AmbientLight_RecOneDataEventHandle(float data, bool Invaild, string msg)
        {
            ...
            if (Program.Language == "EN")
                m_viewResult.AmbientLight = string.Format("{0}", data == 0x01 ? "Light On" : "Light Off");
            else
        ...
        private void BodyState_RecOneDataEventHandle(float data, bool Invaild, string msg)
        {
            ...
            if (Program.Language == "EN")
                m_viewResult.BodySate = string.Format("Position：{0}", data == 0x01 ? "Supination" : data == 0x02 ? "Left Side" : data == 0x03 ? "Right Side" : data == 0x04 ? "Lie Prostrate" : data == 0x05 ? "Stand" : "Handstand");
           else
        ...
        {
            EstimateBloodPressure_spo2 = data;

            if (Program.Language == "EN")
            { 
                msg = "Module Fault";
                m_viewResult.BloodOxygen = string.Format("Oxygen：{0}", Invaild ? msg : string.Format("{0}%", data));
            }
            else
                    ...
                    Task.Factory.StartNew(() =>
                    {
                        ...
                        //m_Protocol.StopSampleRequest();
                        {
                            ...
                            this.Invoke(new MethodInvoker(() =>
                            {
                                ...
                                AhDung.MessageTip.ShowOk(Program.Language=="EN"? "Data collection stopped and execution succeeded!" : "数据停止采集执行成功!");
                            }));
                        }
                        else
                        {
                            AhDung.MessageTip.ShowError(Program.Language == "EN" ? "Data collection stopped and execution failed!" : "数据停止采集执行失败!");
                ...
                Task.Factory.StartNew(() =>
                {
                    ...
                    // m_Protocol.StartSampleRequest();
                    {
                        ...
                        this.Invoke(new MethodInvoker(() =>
                        {
                            ...
                            m_DeviceRun = true;
                            if (Program.Language == "EN")
                                AhDung.MessageTip.ShowOk("Data collection started successfully!");
                            else
                                AhDung.MessageTip.ShowOk("数据开始采集执行成功!");
                            startruntime = DateTime.Now;
                            if (Program.Language == "EN")
                                label3.Text = string.Format("{1}({0}) {3} {2}years old {5}  @Start：{4}", Channel.Default.Patient.PatientNo, Channel.Default.Patient.PatientName, Channel.Default.Patient.Age, Channel.Default.Patient.Gender, startruntime, devName);
                            else
                    ...
                    {
                        AhDung.MessageTip.ShowError(Program.Language == "EN" ? "Data collection stopped and execution failed!" : "数据开始采集执行失败!");
        ...
        private bool setTime_SetTimeHandler(DateTime start, DateTime end)
        {
            ...
            try
            {
                if (m_DeviceRun)
                {
                    AhDung.MessageTip.ShowWarning(Program.Language == "EN" ? "Please stop collecting first" : "请先停止采集");
                    return false;
                }
                ret = m_Protocol.SetAutoRunTime(start, end);
                if (ret)
                {
                    AhDung.MessageTip.ShowOk(Program.Language == "EN" ? "Timed effective" : "定时生效");
                ...
                {
                    AhDung.MessageTip.ShowError(Program.Language == "EN" ? "Setting failed" : "设定失败");
