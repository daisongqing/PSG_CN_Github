...
namespace AwareTec.Polysmith.UI
{
    public class Channel:IDisposable
    {
        ...
        private string m_defaultChanelPath = "DefaultChannels.cfg";
       ...
       /// </summary>
        ...
        /// 加载计划方案
        /// </summary>
        private void SchemeExecute()
        {
            string Schemepath = string.Format("{0}Scheme", AppDomain.CurrentDomain.BaseDirectory);
            if (Directory.Exists(Schemepath))
            {
                SharpZipLibHelper.UnZip(string.Format("{0}Scheme\\Scheme.zip", AppDomain.CurrentDomain.BaseDirectory), Schemepath, "pps");
                string channelxmlPath = string.Format("{0}\\ChannelProperties.xml", Schemepath);
                if (File.Exists(channelxmlPath))
                {
                    ChannelProperties = new List<Doc_Channel>();
                    XmlDocument doc = new XmlDocument();
                    doc.Load(channelxmlPath);
                    XmlNode rootNode = doc.SelectSingleNode("Data");
                    if (rootNode != null)
                    {
                        XmlNodeList nodes = rootNode.SelectNodes("Channel");
                        System.Reflection.PropertyInfo[] info = typeof(Doc_Channel).GetProperties();
                        foreach (XmlNode node in nodes)
                        {
                            Doc_Channel dochannel = new Doc_Channel();
                            for (int i = 0; i < info.Length; i++)
                            {
                                XmlNode sss = node.SelectSingleNode(info[i].Name);
                                if (sss != null)
                                {
                                    info[i].SetValue(dochannel, Convert.ChangeType(sss.InnerText, info[i].PropertyType));
                                }
                            }
                            ChannelProperties.Add(dochannel);
                        }
                    }
                    File.Delete(channelxmlPath);
                }
                string RenamexmlPath = string.Format("{0}\\ReName.XML", Schemepath);
                if (File.Exists(RenamexmlPath))
                {
                    XmlDocument doc = new XmlDocument();
                    doc.Load(RenamexmlPath);
                    XmlNode rootNode = doc.SelectSingleNode("Edit");
                    if (rootNode != null)
                    {
                        m_reNameList = new List<ReNameUnion>();
                        XmlNodeList nodes = rootNode.SelectNodes("Selection");
                        System.Reflection.PropertyInfo[] info = typeof(ReNameUnion).GetProperties();
                        foreach (XmlNode node in nodes)
                        {
                            ReNameUnion uion = new ReNameUnion();
                            for (int i = 0; i < info.Length; i++)
                            {
                                XmlNode sss = node.SelectSingleNode(info[i].Name);
                                if (sss != null)
                                {
                                    info[i].SetValue(uion, Convert.ChangeType(sss.InnerText, info[i].PropertyType));
                                }
                            }
                            m_reNameList.Add(uion);
                        }
                    }
                    File.Delete(RenamexmlPath);
                }
            }
        }
        /// <summary>
        ...
        public Channel()
        {
            m_DefineData = ChannelManage.Default.CreatEmptyTable();
            string defaultChannePath = Path.Combine(ChannelManage.Default.ConfigruationBasePath, m_defaultChanelPath);
            m_DefaultChannel = ChannelManage.Default.ReadChannelConfig(defaultChannePath);
            ...
            SchemeExecute();
            AnalysisReult = new DataModel.AnalysisResult();
            BaseTimeLineSpan = 30;
            ...
            if (ChannelProperties == null)
                ChannelProperties = DataModel.DataBaseHelper.Default.SelectAll(new Doc_Channel());
            if (m_DefaultChannel.Rows.Count == 0)
            {
                string[] files = Directory.GetFiles(ChannelManage.Default.ConfigruationBasePath);
                if (files.Length > 0)
                    m_DefaultChannel = ChannelManage.Default.ReadChannelConfig(files[0]);
                else
                    m_DefaultChannel = CreatChannels(ChannelProperties);
            }
            for (int i = 0; i < ChannelProperties.Count; i++)
            {
                Doc_Channel channel = ChannelProperties[i];
                if (!channel.Enable)
                    continue;
                int len = channel.strPixelRangArray.Length;
                channel.intPixelRangArray = new int[len];
                for (int j = 0; j < len; j++)
                {
                    channel.intPixelRangArray[j] = ChannelTable.getSensitivity(channel.strPixelRangArray[j]); ;
                }
            }
            ReportStructurals = DataModel.DataBaseHelper.Default.SelectAll(new Doc_ReportStructural());
        ...
        public Dictionary<string, string> getReNameMap()
        {
            ...
            if (m_reNameList.Count > 0)
            {
                string[] val = Channel.Default.ArousalDataSourcePlan.Split(';');
                string[] val2 = Channel.Default.SleepStageSourcePlan.Split(';');
                string[] val3 = new string[val.Length + val2.Length];
                Array.Copy(val, 0, val3, 0, val.Length);
                Array.Copy(val2, 0, val3, val.Length, val2.Length);
                var sss = val.Distinct();
                foreach (string ss in sss)
                {
                    ReNameUnion find = m_reNameList.Find(t => t.ID.ToString() == ss);
                    if (find != null)
                    {
                        if (!ret.Keys.Contains(find.OldName))
                        {
                            ret.Add(find.OldName, find.NewName);
                        }
                    }
                }
            }
        ...
        /// <param name="ID"></param>
        /// <returns></returns>
        public bool ShouldRename(ref string Name,int ID)
        {
          ReNameUnion find=  m_reNameList.Find(t=>t.ID==ID);
          if (find != null)
          {
              Name = find.NewName;
              return true;
        ...
        public void LoadVedio(string exeName,string MatchKey,string UrlPath,string savedirectory,bool NotSplitRecordFile,int MaxRecodFileLenght, bool newVedio = false)
        {
            ...
            if (File.Exists(app))
            {
                ...
                if(pro.Start()) //启动
                {
                    DataModel.LogInstance.Default.AddLog(string.Format("视频程序加载成功"), pSystem.LogManagement.LogLevel.WARN);
                }
                else
                {
                    DataModel.LogInstance.Default.AddLog(string.Format("视频程序加载失败"), pSystem.LogManagement.LogLevel.ERROR);
                }
            }
            else
            {
                DataModel.LogInstance.Default.AddLog(string.Format("视频程序地址错误 查询地址为{0}", app), pSystem.LogManagement.LogLevel.ERROR);
            }
        }
        /// <summary>
        /// 资源释放
        ...
        /// 是否处于正在监控状态
        /// </summary>
        public DataModel.ProgressState ProgressStauts { set; get; }
        /// <summary>
        /// 鼻压力通道参数存储
        /// </summary>
        public List<float> PressureMuzzleFlowChannel = new List<float>();
        /// <summary>
        /// 热敏通道数据展示
        /// </summary>
        //public List<float> ThermalMuzzleFlowChannel = new List<float>();
        /// <summary>
        /// 临时通道参数存储
        /// </summary>
        ...
        {
           ChannelTable ret= CurrentSaveTable.Find(t=>t.ID==ID);
           if (ret == null)
           {
              DataRow[] dr= CurrentChannelTable.Select(string.Format("ID='{0}'",ID));
              if (dr.Length > 0)
              {
                 ret= ChannelTable.ConvertToChannel(dr[0]);
              }
           }
           return ret;
        }
        ...
        public string DefaultChannelPath
        {
            get
            {
                return Path.Combine( ChannelManage.Default.ConfigruationBasePath ,m_defaultChanelPath);
            }
        }
        /// <summary>
        /// 自动识别事件开关
        ...
        private Doc_SystemSetting _SystemSetting;
        /// <summary>
        /// 全局设置参数
        /// </summary>
        ...
        /// 获取报告结构图可变元素集合
        /// </summary>
        public List<Doc_ReportStructural> ReportStructurals { get; private set; }
        /// <summary>
        /// 获取通道配置属性列表
        /// </summary>
        public List<Doc_Channel> ChannelProperties { get; private set; }
        /// <summary>
        /// 定标用的关键字
        /// </summary>
        ...
        ///  解析状态字
        /// </summary>
        public DataModel.AnalysisResult AnalysisReult { set; get; }
        /// <summary>
        /// 主页面表格是否需要更新
        /// </summary>
        ...
        private bool m_CanRecover = false;
        public delegate void RecoverStateChangeDelegate(bool recoverstate);
        public event RecoverStateChangeDelegate RecoverStateChangedHandler;
        /// <summary>
        /// 回退锁定
        /// </summary>
        public bool CanRecover
        {
            set
            {
                if (value != m_CanRecover && RecoverStateChangedHandler != null)
                {
                    m_CanRecover = value;
                    RecoverStateChangedHandler.Invoke(value);
                }
            }
            get
            {
        ...
        /// <param name="dt"></param>
        /// <returns></returns>
        public DataTable CreatChannels(List<Doc_Channel> channels)
        {
            DataTable ret = ChannelManage.Default.CreatEmptyTable();
            int m_channelCnt = 0;
            foreach (DataBaseCom.Doc_Channel channel in channels)
            {
                if (!channel.Enable)
                    continue;
                ChannelTable table = new ChannelTable()
                    {
                        ...
                        Visible = true,
                        TimeSpan = channel.SpanTime,
                        ChannelNo = m_channelCnt,
                        ValueZoomRate = channel.ValueZoomRate,
                        PixelRate =channel.PixelEnable?1: 0,
                        HighPass = "Off",
                        LowPass = "Off",
                        SingleNotch = "Off",
                        PenColor = channel.Color,
                        Name = channel.Description,
                        ID = channel.Name,
                        PixelEnable = channel.PixelEnable,
                        CalibrationsVisible=channel.CalibrationsVisible,
                        IsShowValue=channel.IsShowValue,
                        Antipole=channel.AntipoleEnable,
                    };
        ...
        {
            DataTable dt = ChannelManage.Default.CreatEmptyTable();
        ...
        private float[] item_FiliterDataEvent(pChart.CurveItem item, float[] datasource, int firstDataIndex,object filiter)
        {
            ...
            int span = getSampleSpan(item.ID);
        ...
        public CurveItem CreatChannel(ChannelTable table)
        {
            ...
            item.Visible = table.Visible;
            item.yAxis.MaxValue = table.MaxValue;
            item.yAxis.MinValue = table.MinValue;
            item.TimeSpan = 1000 / table.TimeSpan;
            item.ChannelNo = table.ChannelNo;
            item.PenColor = table.PenColor;
            ...
            item.ChannelNum = table.ChannelNum;
            item.PixelRate = table.PixelRate;
            item.belong = table.Belong;
            ...
            item.ValueZoomRate = table.ValueZoomRate;
            item.FiliterDataEvent += item_FiliterDataEvent;
            item.Tag = m_ChannelFiliter.CreatInstance(table.TimeSpan);
            return item;
        }
        /// <summary>
        ...
        /// <param name="typ"></param>
        /// <returns></returns>
        public string[] channel_FilterDropDownHandle(string ID, string typ)
        {
        ...
        /// <param name="ID"></param>
        /// <returns></returns>
        private int getSampleSpan(string ID)
        {
            int span = 0;
            ID = ID.Replace("Clone_", "");
            if (ID.Contains("Append_"))
                ID = ID.Replace("Append_", "").Split(':')[0];
            Doc_Channel find = ChannelProperties.Find(t => t.Name == ID);
            if (find != null)
                span = find.SpanTime;
            return span;
        }
        ...
        /// </summary>
        /// <returns></returns>
        public Image CreatMap(DataModel.ResultDomain result = null, bool breathOnly = false)
        {
            if (result == null)
            ...
            if (reportmaps.Count() > 0)
            {
                ...
                {
                    if ((report.ID == 9 || report.ID == 10 || report.ID == 12 || report.ID == 13) && breathOnly)///ID为9是睡眠分期结构图 10腿动 12 觉醒 13 周期性循环腿动
                    {
                        continue;
                    }
                    Area area = new Area();
            ...
            //    {
            //        case 0:
            //            DataRow[] dr = dt.Select(string.Format("ID='{0}'", area.ID));
            //            if (dr.Length > 0)
            //            {
            //                area.yAxis.MaxValue = Convert.ToSingle(dr[0]["MaxValue"]);
            //                area.yAxis.MinValue = Convert.ToSingle(dr[0]["MinValue"]); ;
            //            }
            //            else
            //            {
            ...
            //            area.Points = result.BloodOxygenPoints.ToList();
            //            area.yAxis.Interval = 5;
            //            area.yAxis.LegendLables = area.yAxis.Calibrations.Select(t => t.ToString()).ToList();
            //            break;
            //        case 1:
            //            DataRow[] dr2 = dt.Select(string.Format("ID='{0}'", area.ID));
            //            if (dr2.Length > 0)
            //            {
            //                area.yAxis.MaxValue = Convert.ToSingle(dr2[0]["MaxValue"]);
            //                area.yAxis.MinValue = Convert.ToSingle(dr2[0]["MinValue"]); ;
            //            }
            //            else
            //            {
            ...
            //            area.Points = result.HeartRatePoints.ToList();
            ...
            //            area.Points = result.CAPoints.ToList();
            ...
            //            area.Points = result.OAPoints.ToList();
            //            area.yAxis.Interval = 2;
            //            area.yAxis.MaxValue = 30;
            //            area.yAxis.MinValue = 0;
            //            area.yAxis.LegendLables = area.yAxis.Calibrations.Select(t => t.ToString()).ToList();
            ...
            //            area.Hight = 80;
            //            area.PenColor = Color.Cyan;
            //            area.Points = result.MAPoints.ToList();
            //            area.yAxis.Interval = 2;
            //            area.yAxis.MaxValue = 30;
            //            area.yAxis.MinValue = 0;
            //            area.yAxis.LegendLables = area.yAxis.Calibrations.Select(t => t.ToString()).ToList();
            ...
            //            area.Points = result.HypopneaPoints.ToList();
            //            area.yAxis.Interval = 2;
            //            area.yAxis.MaxValue = 30;
            //            area.yAxis.MinValue = 0;
            //            area.yAxis.LegendLables = area.yAxis.Calibrations.Select(t => t.ToString()).ToList();
            ...
            //            area.Points = result.MTPoints.ToList();
            //            area.yAxis.Interval = 5;
            ...
            //            area.Points = result.BodyStatePoints.ToList();
            //            area.yAxis.Interval = 4;
            //            area.yAxis.MaxValue = 5;
            //            area.yAxis.MinValue = 1;
            //            area.yAxis.LegendLables = new List<string>() { "S", "L", "R", "P", "UP" };
            //            area.yAxis.CalibrationsColors = new List<Color>() { Color.Olive, Color.LightGreen, Color.DarkCyan, Color.Gold, Color.DimGray };
            //            break;
            //        case 8:
            //            area.Hight = 90;
            //            area.PenColor = Color.SlateGray;
            //            area.Points = result.SleepStagPoints.ToList();
            //            area.yAxis.Interval = 4;
            //            area.yAxis.MaxValue = 5;
            //            area.yAxis.MinValue = 1;
            ...
            #region 指标配图生成
            ///string[] BarMapList = new string[] { Patient.LessSpO2PhotoPath, Patient.RangeSpO2PhotoPath, Patient.RangeHeartRatePhotoPath, Patient.BreathBarPhotoPath, Patient.SymptomDegreePhotoPath };
            #region 程度递归柱状图
            BarChart bar = new BarChart();
            bar.Docksize = 15;
            bar.xAxis.SetMaxMinValue(6, 0);
            bar.xAxis.Interval = 6;
            bar.xAxis.CalibrationsVisible = true;
            bar.xAxis.Name = "";
            bar.yAxis.Name = "";
            bar.xAxis.LegendLables = new List<string> { " ", "AHI", "", "打鼾", "", "SpO2", " " };
            bar.yAxis.SetMaxMinValue(30, 0);
            bar.yAxis.Interval = 2;
            bar.yAxis.CalibrationsVisible = false;
            bar.yAxis.CalibrationsColors = new List<Color>() { Color.Green, Color.Yellow, Color.Orange, Color.Red };
            bar.yAxis.CalibrationsColorsName = new List<string>() { "正常", "轻度", "中度", "重度" };
            bar.Width = 584;
            bar.Hight = 135;
            bar.IsRange = true;
            float[] ydatas = new float[] { Channel.Default.AnalysisReult.BreathEvent.ApneaAndHypopneaIndex, Channel.Default.AnalysisReult.BodyState.SnoreIndex, Channel.Default.AnalysisReult.BloodOxygen.OxygenReduceIndex };
            float[] degreedatas = new float[] { Channel.Default.AnalysisReult.BreathEvent.ApneaHypopneaDegreeLevel, Channel.Default.AnalysisReult.BreathEvent.ApneaHypopneaDegreeLevel, Channel.Default.AnalysisReult.BloodOxygen.BloodOxygenLowDegreeLevel };
            for (int i = 0; i < ydatas.Length;i++)
            {
                SuperPointF sp = new SuperPointF();
                sp.XValue = 2 * i;
                sp.YMaxValue = ydatas[i];
                sp.YMinValue = degreedatas[i];
                bar.Points.Add(sp);
                SuperPointF sp2 = new SuperPointF();
                sp2.XValue = 2 * (i + 1);
                sp2.YMaxValue = ydatas[i];
                sp2.YMinValue = sp.YMinValue;
                bar.Points.Add(sp2);
            }
            bar.GetMap().Save(Patient.SymptomDegreePhotoPath);
            #endregion

            #region 血氧值递归曲线图
            //BarChart lessbar = new BarChart();
            //lessbar.xAxis.SetMaxMinValue(60, 0);
            //lessbar.xAxis.Interval = 6;
            //lessbar.xAxis.CalibrationsVisible = false;
            //lessbar.xAxis.LegendLables = lessbar.xAxis.Calibrations.Select(t => string.Format("{0}%", 100 - t)).ToList();
            //lessbar.yAxis.SetMaxMinValue(100, 0);
            //lessbar.yAxis.Interval = 5;
            //lessbar.yAxis.CalibrationsVisible = true;
            //lessbar.yAxis.LegendLables = lessbar.yAxis.Calibrations.Select(t => string.Format("{0}%", t)).ToList();
            //lessbar.Width = 510;
            //lessbar.Hight = 165;
            //lessbar.xAxis.Name = "% SpO2";
            //lessbar.yAxis.Name = "% 睡眠时间";
            //lessbar.IsRange = false;
            //for (int i = 0; i < Channel.Default.AnalysisReult.BloodOxygen.SpO2LevelTotalTimes.Count; i++)
            //{
            //    float y =(float)Math.Round( Channel.Default.AnalysisReult.BloodOxygen.SpO2LevelTotalTimes[i]*100 / (60 * Channel.Default.AnalysisReult.Sleep.TimeInBed),2);
            //    SuperPointF sp = new SuperPointF();
            //    sp.XValue = i;
            //    sp.YMaxValue = y;
            //    lessbar.Points.Add(sp);
            //}
            //lessbar.GetMap().Save(Patient.LessSpO2PhotoPath);
            #endregion
            #endregion
            ...
            for (int i = 0; i < result.MultSleepRecords.Count; i++)
            {
                if (i < sleepMapList.Length)
                {
                    ...
                    {
                        DataModel.LogInstance.Default.AddLog(string.Format("生成谱图失败 {0}", ee.StackTrace), pSystem.LogManagement.LogLevel.ERROR);
                    }
                }
            }
            return ret;
