    ...
    public class EDF:DataCenter.BaseFile
    {
        ...
        public EDF(byte[] m_headBytes,long dataLength)
        {
            ...
            for (int i = 0; i < this.DataSignCnt; i++)
            {
                ...
                channel.ID = -1;
                DataBaseCom.Doc_Channel find = Channel.Default.ChannelProperties.Find(t => t.SignName.ToLower() == channel.DataSignName);
                if (find != null)
                    channel.ID = find.ID;
        ...
        {
          DataBaseCom.Doc_Channel find=  Channel.Default.ChannelProperties.Find(t=>t.SignName.ToLower()==ChannelName.ToLower());
          if (find != null)
          {
              return find.Name;
          }
        ...
        /// <param name="ChannelName"></param>
        /// <returns></returns>
        public string ConvertToChannelIDEx(int ChannelID)
        {
            DataBaseCom.Doc_Channel find = Channel.Default.ChannelProperties.Find(t => t.ID == ChannelID);
            if (find != null)
            {
                return find.Name;
            }
        ...
        public ChannelItem ConvertToChannelItemEx(string ID, int TimeSpan)
        {
            ...
            ID = ID.Replace("Clone_", "").Replace("Append_", "").Split(':')[0];
