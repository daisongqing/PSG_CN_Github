...
namespace AwareTec.Polysmith.UI.DataModel
{
    public class RemoteDataInteration : ICommunication
    {
        ...
        /// 通讯端口
        /// </summary>
        private int m_PortID = int.Parse(ReadTxt()[1]);
        /// <summary>
        /// 帧集合
        /// </summary>
        ...
        public void getEvents(string value, string guid)
        {
            ...
            for (int j = 0; j < strRec.Length; j++)
            {
                ...
                if (ss.Length == 3)
                {
                    ...
                    int TimeSpan = 0;
                    #region 注释掉
                    //pChart.IMarker mark = MarkerManage.Default.DefineMarkers.Find(t => (int)t.MarkTyp == typ);
                    //if (mark != null)
                    //{
                    //    if (mark.MarkTyp == pChart.IMarker.MarkType.CheyneStokes)///陈施氏呼吸事件暂时不显示，由医生手动标记
                    //        continue;
                    //    string des=mark.Description;
                    //    if (mark is pChart.RectangleMarkers)
                    //    {
                    //        ss2 = (mark as pChart.RectangleMarkers).OnChannelIndexs;
                    //    }
                    //    if (mark.MarkTyp == pChart.IMarker.MarkType.OxygenReduce)
                    //    {
                    //        des = des.Replace(string.Format("{0}%", Channel.Default.SystemSetting.OxygenReduceRange), string.Format("{0}%", Channel.Default.OxygenReduceRange));
                    //    }
                    ...
                    //        for (int s = 0; s < ss2.Length; s++)
                    //        {
                    //            Doc_EventRecords rec = new Doc_EventRecords();
                    //            rec.StartIndex = sindex;
                    //            rec.EndIndex = eindex;
                    //            rec.EventType = typ;
                    //            rec.EventName = mark == null ? typ.ToString() : mark.Name;
                    //            rec.Description = des;
                    //            rec.ChannelID = ss2[s];
                    //            rec.TimeSpan = TimeSpan;
                    //            rec.Tag = stridx.Length == 3 ? stridx[2] : "";
                    //            rec.EventID = string.Format("{0}:{1}-{2}", ss2[s], rec.StartIndex, rec.EndIndex);
                    //            rec.GUID = guid;
                    //            m_EventReocrds.Add(rec);
                    //        }
                    //    }
                    //}
                    #endregion
        ...
        private bool SendData(string data)
        {
            ...
            {
                DataModel.LogInstance.Default.AddLog(string.Format("发送到AI：{0}", data),pSystem.LogManagement.LogLevel.FATAL);
                int maxlenght = 1000;
                int len = data.Length;
            ...
            try
            {
                ...
                switch (typ)
                {
                    case DataType.BaseInfo:
                        DataModel.LogInstance.Default.AddLog(string.Format("数据接受 {0}", m_recDataString), pSystem.LogManagement.LogLevel.FATAL);
        ...
        public bool SubmitAnalysisConditions(InConditions Conditions)
        {
            ...
            m_eventsReadComplete = (1 << (int)pChart.IMarker.MarkType.Stage) == Channel.Default.AnalysisStateWord;
            ...
            foreach (int value in names)
            {
                if (value < 50)
                {
                    int on = (Channel.Default.AnalysisStateWord >> value) & 1;
        ...
        private static string m_AnalysisExePath = "";
        /// <summary>
        /// 开启线程
        /// </summary>
        /// <returns></returns>
        ...
        private static List<string> ReadTxt()
        {
            ...
            string line;
            System.IO.StreamReader file =new System.IO.StreamReader($"{Path.GetDirectoryName(string.Format("{0}AnalysisSource\\", AppDomain.CurrentDomain.BaseDirectory))}\\ip_config.txt");
