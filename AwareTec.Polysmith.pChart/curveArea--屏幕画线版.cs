...
namespace AwareTec.Polysmith.pChart
{
    public partial class curveArea : UserControl
    {
        ...
        private void view_Paint(object sender, PaintEventArgs e)
        {
            ...
            m_freshOk = true;
            m_hasLine = false;
            Console.WriteLine(string.Format("趋势图{1}容器执行一次曲线绘制，耗时[{0}ms]", (DateTime.Now - ddt).TotalMilliseconds, Belong == 0 ? "主" : "副"));
            m_userRefresh = false;
        }
        ...
        private void view_MouseUp(object sender, MouseEventArgs e)
        {
            ...
            if (!m_isRealDataCurve)
            {
                ...
                if (m_SelectMark != null)
                {
                    ...
                    m_hasLine = true;/// 为了满足panel2的重绘条件，临时加
                    toolTip1.Hide(this);
                    toolTip1.RemoveAll();
        ...
        private void view_MouseMove(object sender, MouseEventArgs e)
        {
            ...
            if (isContains)
            {
                ...
                {
                    Point cur = panel2.PointToScreen(e.Location);
                    int offset = e.Location.Y - cur.Y;
                    if (bak_mousePoint == cur)
                        return;
                    if (bak_mousePoint.X != e.Location.X && bak_mousePoint.Y != e.Location.Y && bak_mousePoint.X > 0 && m_hasLine)
                    {
                        ControlPaint.DrawReversibleLine(new Point(bak_mousePoint.X, ClientRectangle.Top - offset), new Point(bak_mousePoint.X, ClientRectangle.Bottom - offset), Color.DarkGray);
                        m_hasLine = false;
                    }
                    if (!m_hasLine)
                    {
                        ControlPaint.DrawReversibleLine(new Point(cur.X, ClientRectangle.Top - offset), new Point(cur.X, ClientRectangle.Bottom - offset), Color.DarkGray);
                        m_hasLine = true;
                    }
                    bak_mousePoint = cur;
                    if (TimeLineVisible)
        ...
        {
            {
                ...
                if (m_TimeLineVisible && m_hasLine)
                {
                    int offset = e.Location.Y - panel2.PointToScreen(e.Location).Y;
                    ControlPaint.DrawReversibleLine(new Point(bak_mousePoint.X, ClientRectangle.Top - offset), new Point(bak_mousePoint.X, ClientRectangle.Bottom - offset), Color.DarkGray);
                    bak_mousePoint = new Point(-10, 0);
                    m_hasLine = false;
                }
