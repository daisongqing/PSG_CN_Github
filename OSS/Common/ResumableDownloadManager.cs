    ...
    internal class ResumableDownloadManager
    {
                                                    ...
                                                    EventHandler<StreamTransferProgressArgs> downloadProgressCallback)
        {
            ...
            try
            {
                ...
                while (nextPart < resumableContext.PartContextList.Count)
                {
                    ...
                    if (taskParams[index].Error == null)
                    {
                        if (request.StreamTransferProgress != null)
                        {
                            ...
                            StreamTransferProgressArgs args = new StreamTransferProgressArgs(resumableContext.PartContextList[index].Length, lastDownloadedBytes, totalBytes) { Tag = _ossClient.Tag };
                ...
                if (request.StreamTransferProgress != null)
                {
                    StreamTransferProgressArgs args = new StreamTransferProgressArgs(_downloadedBytes - lastDownloadedBytes, _downloadedBytes, totalBytes) { Tag = _ossClient.Tag };
                    request.StreamTransferProgress.Invoke(this, args);
                }
