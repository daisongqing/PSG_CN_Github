...
namespace pSystem.DataBaseHepler
{
    ...
    /// MySQL数据库操作对象
    /// </summary>
    public class MySQLHepler : IDataBaseCommand, ISessionFactory
    {
        /// <summary>
        /// SQL连接对象
        /// </summary>
        private string sqlconstr = "server=47.110.8.224;port=3306;user=psgdb;password=Psg123456; database=leasedatabase;";
        ...
        /// 构造函数
        /// </summary>
        public MySQLHepler(string SqlConnection)
        {
            if (SqlConnection != "")
        ...
        {
            using (var m_sqlcon = CreateSession())
            {
                using (IDbTransaction trans = m_sqlcon.BeginTrans())
                {
                    using (IDbCommand cmd = m_sqlcon.Connection.CreateCommand())
                    {
        ...
        public List<string> GetAllTableName()
        {
            ...
            using (var session = CreateSession())
            {
                DataTable tbName = (session.Connection as MySqlConnection).GetSchema("Tables");
        ...
        private class mySqlDbSession : DbSession
        {
            public mySqlDbSession(IDbConnection conn)
                : base(conn)
            {

            }
            public mySqlDbSession(IDbConnection conn, IDbTransaction trans)
                :base(conn, trans)
            {

            }
            public event Action<mySqlDbSession> Disposing;
            /// <summary>
            /// 重新写资源释放方法
            /// </summary>
        ...
        /// 
        /// </summary>
        private ConcurrentQueue<IDbConnection> m_dbConnections = new ConcurrentQueue<IDbConnection>();
        /// <summary>
        /// 最大数据库连接数限制
        /// </summary>
        private int m_MaxConnCount = 50;
        private void myDispose(mySqlDbSession session)
        {
            if (m_dbConnections.Count > m_MaxConnCount)
            {
                session.Close();
            }
            else
            {
                m_dbConnections.Enqueue(session.Connection);
            }
        }
        /// <summary>
        /// 创建数据库连接
        ...
        {
            IDbConnection m_sqlcon;
            if (!m_dbConnections.TryDequeue(out m_sqlcon))
            {
                m_sqlcon = new MySqlConnection(sqlconstr);
            }
        ...
        public IDbSession CreateSession()
        {
            ...
            mySqlDbSession session = new mySqlDbSession(conn);
            session.Disposing += myDispose;
            return session;
        }
        ...
        {
            mySqlDbSession session = new mySqlDbSession(conn, trans);
            session.Disposing += myDispose;
            return session;
        }
        #endregion
