...
namespace pSystem.DataBaseHepler
{
    ...
    /// SQL数据库操作对象
    /// </summary>
    public class SQLiteHepler : IDataBaseCommand, ISessionFactory
    {
        ...
        private IDbSession m_Session = null;
        private DataTable DataRead(string cmd)
        {
            try
            {
                DataTable dt = new System.Data.DataTable();
                if (m_Session.Connection.State == ConnectionState.Closed)
                {
                    m_Session.Connection.Open();
                    m_Session.BeginTrans();
                }
                DateTime time = DateTime.Now;
                try
                {
                    IDataReader dreader = m_Session.Connection.ExecuteReader(cmd, null, m_Session.Transaction);
                    DataTable schemaTable = dreader.GetSchemaTable();
                    Console.WriteLine(string.Format("****数据库查询耗时{0}ms", (DateTime.Now - time).TotalMilliseconds));
                    //动态构建表，添加列
        ...
        /// 构造函数
        /// </summary>
        public SQLiteHepler(string SqlConnection)
        {
            if (SqlConnection != "")
        ...
        {
            string sqlcommandstring = string.Format("SELECT * FROM {0}", Condition.GetThisTableName());
        ...
        public DataTable Select(ITable Condition, string OrderBy)
        {
            ...
            if (OrderBy != "")
            {
                sqlcommandstring = string.Format("{0} ORDER BY {1} ASC", sqlcommandstring, OrderBy);
            }
            return (DataRead(sqlcommandstring));
        ...
        public DataTable Select(ITable Condition, string OrderBy, int TopCount)
        {
            ...
            if (OrderBy != "")
            {
                sqlcommandstring = string.Format("{0} ORDER BY {1} DESC LIMIT {2}", sqlcommandstring, OrderBy, TopCount);
            }
            return (DataRead(sqlcommandstring));
        }
        /// <summary>
        ...
        {
            string sqlcommandstring = string.Format("DELETE FROM {0}", Condition.GetThisTableName());
            string afterstring = Condition.GetSelectString();
            if (afterstring != "")
            {
                sqlcommandstring = string.Format("{0}\r\n WHERE {1}", sqlcommandstring, afterstring);
            }
        ...
        {
            using (SQLiteConnection m_sqlcon = CreateConnection() as SQLiteConnection)
            {
                using (System.Data.SQLite.SQLiteTransaction trans = m_sqlcon.BeginTransaction())
                {
                    using (SQLiteCommand cmd = new SQLiteCommand(m_sqlcon))
                    {
                ...
                using (System.Data.SQLite.SQLiteTransaction trans = m_sqlcon.BeginTransaction())
                {
                    using (SQLiteCommand cmd = new SQLiteCommand(m_sqlcon))
                    {
                        ...
                        try
                        {
                            ...
                            for (int i = 0; i < Value.Count; i++)
                            {
                                string sqlcommandstring = string.Format("INSERT INTO {0} ", Value[i].GetThisTableName());
                                string afterstring = Value[i].GetInsertString();
                                if (afterstring == "")
                                    return (false);
                                sqlcommandstring = string.Format("{0}{1}", sqlcommandstring, afterstring);
                                cmd.CommandText = sqlcommandstring;
                                cmd.ExecuteNonQuery();
                                cnt++;
                            }
        ...
        {
            string sqlcommandstring = string.Format("UPDATE {0}", OldValue.GetThisTableName());
                ...
                using (System.Data.SQLite.SQLiteTransaction trans = m_sqlcon.BeginTransaction())
                {
                    using (SQLiteCommand cmd = new SQLiteCommand(m_sqlcon))
                    {
                        ...
                        try
                        {
                            ...
                            {
                                string sqlcommandstring = string.Format("UPDATE {0}", p.Value.GetThisTableName());
        ...
        public List<string> GetAllTableName()
        {
            ...
            DataTable tbName = (CreateConnection() as SQLiteConnection).GetSchema("Tables");
            if (tbName.Columns.Contains("TABLE_NAME"))
            {
                foreach (DataRow dr in tbName.Rows)
                {
                    if (dr["TABLE_TYPE"] != "SYSTEM_TABLE")
        ...
        {
            IDbConnection m_sqlcon = new SQLiteConnection(sqlconstr);
